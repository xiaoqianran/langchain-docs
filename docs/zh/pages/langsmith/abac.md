...
拒绝访问包含 PII 的数据集。由于拒绝策略会覆盖允许策略，因此即使具有 RBAC 权限的用户也会阻止访问：
...
<Note>
如果您在跟踪提取期间依赖 LangSmith SDK 自动创建跟踪项目，则 `tag_value_ids` 参数在该自动创建路径上不可用。为了确保 ABAC 策略从一开始就适用，请在开始跟踪会话之前通过 `POST /api/v1/sessions` 使用所需的 `tag_value_ids` 预先创建项目。
</Note>
...
- 一组中有多个条件，所有条件都必须匹配（AND 逻辑）**意外授予访问权限？**
- 检查 RBAC 权限（用户可以通过其角色进行访问）
