...
<Accordion title="Previous v0.11 releases">
  ...
  <Update label="2026-07-08">
    ...
    * 修复了当 API 服务器和队列工作线程作为同一 Kubernetes pod 中的单独容器运行时，JS 工作线程端口冲突。队列入口点现在偏移环回端口。
  ...
  <Update label="2026-07-08">
    ...
    ### 新功能* 添加了 Wolfi Python 和 JS 服务器映像的 `-fips` 变体（例如 `3.13-wolfi-fips`、`22-wolfi-fips`），使用 Go FIPS 140 加密模块和适用于 Node.js 的 FIPS 强化 OpenSSL 构建。
  </Update>
  ...
  <Update label="2026-06-30">
    ...
    ### 一般说明* 代理服务器指标现在通过专用 Prometheus 抓取端点上的 OpenTelemetry/Prometheus 客户端发出（`LSD_PROM_METRICS_PORT`，默认 9464）。设置 `LSD_PROM_METRICS_ENABLED=true` 以启用端点，设置 `EXPOSE_INTERNAL_METRICS_PROMETHEUS=true` 以公开从主 API `/metrics` 路径迁移的内部指标。默认情况下，Prometheus 端点仅提供 LSD 部署 UI 指标。
  ...
  <Update label="2026-06-26">
    ...
    ### 新功能* 添加了速率限制可观测性指标，包括配置限制指标（`lg_api_rate_limit_configured_rate`、`lg_api_rate_limit_configured_burst`）以及决策、错误和成本指标上的每个存储桶`rate_limit_key`标签。
  </Update>
...
<Update label="2026-05-11">
  ...
  * 添加了 Postgres 检查点池调整旋钮，适用于一次加载大量大型检查点的情况。现在可以设置 LANGGRAPH\_CHECKPOINTER\_POSTGRES\_POOL\_MIN\_SIZE 和 LANGGRAPH\_CHECKPOINTER\_POSTGRES\_POOL\_TIMEOUT\_SECONDS。
  * 修复了当线程没有先前检查点时 mongo 检查点中的 update\_state 崩溃的问题。
  * 包括针对安全漏洞的依赖项更新。

  ### 新功能

  #### 达美渠道支持现在支持增量通道，因此检查点可以存储增量状态更新，而不是重复存储完整通道有效负载，这有助于处理大型、重附加状态（例如消息历史记录）。
...
<Update label="2026-04-15">
  ...
  * 解决了 checkpoint\_delete\_queue 的迁移版本冲突，确保正确执行并为将来的迁移添加了重复版本检测。
</Update>
...
<Update label="2026-04-09">
  ...
  * 更新了 OpenAPI 配置，以防止在使用带有路径前缀的 Istio 时，`/docs`“尝试一下”请求中出现 405 错误。
...
<Update label="2026-04-06">
  ## v0.7.98* 修复了`langgraph dev`中的导入问题，以确保开发服务器在没有环境变量的情况下工作，并添加了回归测试。
</Update>
...
<Update label="2026-03-31">
  ## v0.7.94* 解决了重试超时后 JavaScript 安装错误地成功的问题，确保正确的故障处理。
...
<Update label="2026-03-31">
  ...
  * 通过缓存`importlib.metadata.packages_distributions()`提高导入性能，显着减少将`ddtrace`与Google API包一起使用时的启动时间。
</Update>

<Update label="2026-03-29">
  ## v0.7.91

  * 将加密依赖项从 46.0.5 升级到 46.0.6，以解决与对等名称验证中的名称限制相关的安全问题。
...
<Update label="2026-03-23">
  ...
  * 将嘈杂的警告级别日志降级为信息级别，以减少日志混乱，重点关注信息状态消息，例如许可证精简模式和禁用跟踪。
  * 通过 Orchestrion DD APM 跟踪增强了 Go `core-api-grpc`，以实现自动检测和改进的跟踪上下文传播。
</Update>
...
<Update label="2026-03-13">
  ...
  * 将undici 包从版本7.22.0 更新到7.24.0 以解决多个安全漏洞。
</Update>

<Update label="2026-03-13">
  ## v0.7.71

  * 通过从线程状态检查点和运行创建方法中删除未使用的参数来清理 API。
  ...
  * 增加了对Redis队列自定义加密的支持，增强数据安全性。
</Update>

<Update label="2026-03-11">
  ## v0.7.69

  * 为 crons 添加了可选的 `timezone` 字段，允许在用户指定的时区进行 `next_run_date` 计算，默认为 UTC。
...
<Update label="2026-03-10">
  ...
  * 通过仅提取经过验证的安全成员来确保安全的 tarfile 提取，以防止任意文件写入漏洞。
...
<Update label="2026-03-02">
  ## v0.7.61* 修复了竞争条件，以确保运行状况和指标服务器正常关闭。
</Update>

<Update label="2026-02-27">
  ## v0.7.59

  * 更新了 Redis 队列，将 zset 与线程结合使用，CPU 使用率降低了 25%，并通过消除不必要的锁定和优化索引来提高性能。
</Update>
...
<Update label="2026-02-24">
  ...
  * 添加了对分布式运行时的反馈\_keys 支持，包括使用 langsmith-go SDK 生成预签名的反馈令牌。
...
<Update label="2026-02-22">
  ## v0.7.51* 改进了上游中断期间许可证检查的恢复能力，包括缓存回退、24 小时宽限期以及 Redis 条目的自动清理。
...
<Update label="2026-02-20">
  ...
  * 请求有效负载中保留的元数据键现在会被静默剥离，而不会导致 422 错误，从而增强了用户体验。
...
<Update label="2026-02-13">
  ...
  * 将 A2A 协议支持更新为 v1.0 RC，重命名为 JSON-RPC 方法，添加了 ListTasks 处理程序，并增强了角色、状态和部件格式，以改进集成和合规性。
  * 改进了`Crons.search()`和`Crons.count()`的身份验证过滤，以防止未经授权的线程信息访问。
  * 修复了 BYOC 检查点中用于复制、回滚和命名空间过滤操作的缺陷，确保跨不同存储后端的正确处理。
</Update>
...
<Update label="2026-02-13">
  ...
  * 通过清理单引号 HTML onload 属性中的消息名称，解决了 handle\_ui 端点中存储的 XSS 漏洞。
  * 修复了`put_item`中的授权绕过问题，以确保身份验证处理程序正确重写命名空间。
  * 在运行创建期间强制执行助理所有权检查，防止在无主助理上执行，同时确保所有经过身份验证的用户仍然可以访问系统助理。
  * 使用 LANGGRAPH\_AES\_KEY 时，对检查点 blob 实施 AES 加密并写入 Go 检查点。
...
<Update label="2026-02-11">
  ...
  * 增强的自定义检查点适配器具有新功能和改进的元数据丰富性，以实现一致的 API 响应。
...
<Update label="2026-02-06">
  ## v0.7.24* 优化 Postgres 连接处理，以防止在高负载下达到连接限制，并删除不必要的错误日志。
...
<Update label="2026-02-03">
  ## v0.7.16* 将线程流切换到新的 gRPC 后端以提高性能。
...
<Update label="2026-02-03">
  ...
  * 添加了对使用新的 `is_enabled` 字段暂停 cron 的支持，仅允许执行已启用的 cron 。
...
<Update label="2026-02-02">
  ...
  * 引入了带有单元测试的自定义检查点适配器来验证实现检查。
</Update>

<Update label="2026-01-29">
  ## v0.7.13

  * 修复了设置挂载前缀时无法通过请求正确保留应用程序状态的错误。
</Update>
...
<Update label="2026-01-27">
  ## v0.7.10* 将 gRPC 服务器启动超时增加到 1 分钟，以防止与核心服务器偶尔连接超时。
...
<Update label="2026-01-12">
  ...
  * 统一错误格式，删除 `code` 字段，并将所有错误标准化为返回带有 `detail` 字段的 JSON。
</Update>
...
<Update label="2026-01-09">
  ...
  * 添加了Python gRPC客户端来管理运行操作，增强Go和Python实现之间的一致性。
</Update>
...
<Update label="2026-01-08">
  ...
  * 在执行器层运行 core-api gRPC 服务器以支持图中的环回 API 调用，并删除了用于禁用服务器的不必要配置。
</Update>
...
<Update label="2026-01-05">
  ...
  * 将 Pregel 中的图形配置与调用配置合并，优先调用设置。
  * 向 GET /threads/ 端点引入了 `include=ttl` 查询参数，用于可选的 TTL 信息检索，而不影响标准读取性能。
...
<Update label="2025-12-23">
  ## v0.6.12* 改进了resolve\_embeddings，使其更加健壮，支持多次调用而不会出现错误。
...
<Update label="2025-12-20">
  ...
  * 确保运行取消仅处理“消息”类型的 Redis 事件，提高了 pubsub 客户端的可靠性。
...
<Update label="2025-12-18">
  ## v0.6.7* 记录服务器启动时间以改进监控和诊断。
</Update>

<Update label="2025-12-17">
  ## v0.6.5

  * 添加了在导入期间触发的警告日志，以提高可见性。
</Update>
...
<Update label="2025-12-11">
  ...
  通过确保 ENTERPRISE\_SAAS 全局标志的正确初始化，修复了混合模式下 cron 作业的问题。
</Update>
...
<Update label="2025-11-26">
  ...
  * (JS) `uuid`从版本10.0.0更新至13.0.0。和 `exit-hook` 从版本 4.0.0 到 5.0.1。
</Update>
...
<Update label="2025-11-22">
  ...
  * 将 store\_get 返回类型转换为 JavaScript 中的元组，以确保类型一致性。
</Update>
...
<Update label="2025-11-21">
  ...
  * 禁用 Redis Go 维护通知，以防止在 Redis 8 以下版本中因不支持的命令而导致启动错误。
</Update>
...
<Update label="2025-11-07">
  ...
  * 参考PostgreSQL，实现了Go中运行的CRUD操作，简化了JSON合并并提高了事务可读性。
</Update>
...
<Update label="2025-10-31">
  ## v0.5.0此次要版本现在需要 langgraph-checkpoint 版本高于 3.0，以防止早期版本的 langgraph-checkpoint 库中的反序列化漏洞。
...
<Update label="2025-10-02">
  ## v0.4.35* 解决了核心API中的时区问题，确保时间数据检索准确。
  * 引入了新的 `middleware_order` 设置，在自定义中间件之前应用身份验证中间件，从而可以更好地控制受保护的路由配置。
  * 记录 Redis 客户端创建过程中发生错误时的 Redis URL。
...
<Update label="2025-09-30">
  ## v0.4.30* 添加了使用 `stream_mode="events"` 时对上下文的支持，并包含了针对此功能的新测试。
  * 添加了对使用 `$LANGGRAPH_SERVER_PORT` 覆盖服务器端口的支持，并删除了不必要的 Dockerfile `ARG` 以实现更清晰的配置。
  * 对线程删除CTE中的所有表引用应用授权过滤器以增强安全性。
...
<Update label="2025-09-22">
  ## v0.4.24* 添加了根据 A2A 规范返回 A2A 调用的完整消息历史记录的功能。
  * 在 Dockerfiles 中添加了 `LANGGRAPH_SERVER_HOST` 环境变量，以支持双堆栈模式的自定义主机设置。
</Update>
...
<Update label="2025-09-04">
  ...
  * 优化更新流，防止不必要的数据传输。
  * 将`storage_postgres/langgraph-api-server`中的`hono`从4.9.2版本升级到4.9.6，以提高URL路径解析安全性。
...
<Update label="2025-08-29">
  ...
  * 通过在 Redis 无法唤醒工作线程的情况下添加回退措施来防止队列终止。
</Update>

<Update label="2025-08-28">
  ## v0.4.4* 将无状态运行的自定义身份验证线程_id 设置为 None 以防止冲突。
  * 通过添加唤醒工作线程和 Redis 锁实现，改进了 Go 运行时中的 Redis 信号传输，并更新了扫描逻辑。
</Update>
...
<Update label="2025-08-25">
  ...
  * 在线程流中正确发出尝试消息。
  * 通过在集群映射中仅使用线程 ID 进行散列来减少集群冲突，并通过 Stream\_thread\_cache 优先考虑效率。
...
<Update label="2025-08-25">
  ...
  * 通过更正 shell 命令格式恢复 Wolfi 映像构建，并添加 Makefile 目标以使用 nginx 进行测试。
</Update>

<Update label="2025-08-22">
  ## v0.3.3* 为特定的 Redis 调用添加了超时，以防止工作线程处于活动状态。
...
<Update label="2025-08-21">
  ...
  * 停止将运行可配置值传播到线程配置，因为如果您指定检查点\_id，这可能会导致后续运行出现问题。这是行为上的**轻微破坏性变化**，因为线程值将不再自动反映最近运行的联合配置。然而，我们认为这种行为更直观。
  * 通过处理 ops.py 中通道名称中的事件数据，增强了与旧工作版本的兼容性。
</Update><Update label="2025-08-20">
  ## v0.2.137
...
<Update label="2025-08-18">
  ...
  * 改进了 SSH 功能，以获得更好的可靠性和稳定性。
  * 将 @langchain/langgraph-api 更新到版本 0.0.59 以修复无效状态架构问题。
</Update>

<Update label="2025-08-15">
  ## v0.2.132* 新增Go语言镜像，增强项目兼容性和功能。
...
<Update label="2025-08-13">
  ## v0.2.130* 添加了一个功能标志 (FF\_RICH\_THREADS=false) 以禁用运行创建时的线程更新，减少锁争用并简化线程状态处理。
...
<Update label="2025-08-08">
  ...
  * 对 Runs.stats 函数进行去抖处理，以减少昂贵的调用频率并提高性能。
  * 为清扫机引入了去抖功能，以提高性能和效率（#1147）。
  * 获取 TTL 扫描锁，以防止在横向扩展操作期间出现数据库垃圾邮件。
</Update>
...
<Update label="2025-07-24">
  ...
  * 在 wait 方法中捕获中断事件以保留 langgraph 0.5.0 中以前的行为。
...
<Update label="2025-07-22">
  ...
  * 恢复更改以确保 gen UI 的 UI 命名空间是有效的 JavaScript 属性名称。
...
<Update label="2025-07-16">
  ...
  * 通过省略 langgraph 0.5 及更高版本的待处理发送来提高性能。
  * 改进了服务器启动日志，以便在设置 DD\_API\_KEY 环境变量时提供更清晰的警告。
</Update>
...
<Update label="2025-06-27">
  ...
  * 修复了无限帧循环问题，并删除了由于 structlog 的意外行为而导致的 dict\_parser。
...
<Update label="2025-06-26">
  ...
  修复了使用 `on_not_exist="create"` 创建具有相同线程\_id 的多个运行时的 404 错误。
</Update>
...
<Update label="2025-06-12">
  ...
  * 通过在 thread\_id/running 上添加唯一索引、优化行锁并确保确定性运行选择，增强查询效率和可靠性。
