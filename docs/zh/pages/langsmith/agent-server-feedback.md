...
本教程向您展示如何收集 [Agent Server](/langsmith/agent-server) 运行的用户反馈，并自动将它们链接到 LangSmith 中的 [traces](/langsmith/observability-concepts#traces)。创建运行时，请将键包含在请求正文的 `feedback_keys` 字段中。响应将返回每个密钥的预签名 URL，您的客户端可以使用该 URL 来收集代理服务器运行的用户反馈。

LangSmith 使用反馈来不断改进代理的实施。要了解有关 LangSmith 中反馈如何工作的更多信息，请参阅[LangSmith feedback](/langsmith/observability-concepts#feedback)。
...
创建一个运行并从响应中解析 `feedback` 对象。<Tabs>
...
此请求成功后，LangSmith 使用密钥 `user_disliked` 记录跟踪反馈。
...
* 将 `key="user_score"` 与 `score=1` 一起使用来获得 `user_liked`
* 将 `key="user_score"` 与 `score=-1` 一起使用来获得 `user_disliked`
