...
在 [UI](https://smith.langchain.com?utm_source=docs&utm_medium=cta&utm_campaign=langsmith-signup&utm_content=langsmith-alerts) 中，导航到您要为其配置警报的跟踪项目。单击页面右上角的 **警报** 图标可查看该项目的现有警报并设置新警报。## 步骤 2：选择指标类型
...
**示例：** 当过去 5 分钟内超过 5% 的运行导致错误时，屏幕截图中的配置将生成警报。
...
<Tabs>
  <Tab title="Slack">
    使用 LangSmith 的本机 Slack 集成将警报通知直接发送到 Slack 通道。无需自定义 Webhook 或 Slack 应用程序配置。<Note>
    ...
    **先决条件**
    ...
    ### 通知格式当警报触发时，LangSmith 会发布结构化的 Slack 消息，其中包括：

    - **标题**：警报名称和您的 LangSmith 工作区名称。
    - **详细行**：指标属性、触发值、比较运算符、配置的阈值、聚合方法和时间窗口 - 例如：`Total Cost: $12.50 ≥ $5.00 · avg · 30 min`。
    - **操作按钮**：**查看警报**（链接到LangSmith中的警报预览）和**查看运行**（链接到触发警报的过滤运行）。
  </Tab>
  <Tab title="PagerDuty">
    ...
    **先决条件**
    ...
    ![PagerDuty Setup](/langsmith/images/pagerduty-setup.png)1. 在 LangSmith 警报设置的通知部分中，选择 **PagerDuty**
    2. 单击密钥图标将集成密钥保存为工作区密钥或选择现有的工作区密钥。作为最佳实践，我们建议将集成密钥保存为工作区机密，而不是直接添加。这将允许您在工作区的警报之间重复使用相同的密钥。
    3. 配置附加通知选项：
       - **严重性**：映射到 PagerDuty 事件优先级
    4. 单击 **发送测试警报** 发送测试警报
    5. 验证事件是否由 PagerDuty 触发并包含相关LangSmith警报信息

    ### 故障排除

    如果未在 PagerDuty 中创建事件：- 验证在LangSmith中正确输入集成密钥
  ...
  <Tab title="Dynatrace">
    ...
    **先决条件**
    ...
    1. 导航至 **访问令牌**。
    1. 单击**生成新令牌**。
    1. 提供描述性名称（例如“LangSmith 警报”）。
    1. 在 **范围** 下，搜索并启用 `events.ingest`（摄取事件）。
    1. 单击**生成令牌**。
    1. 复制生成的token并安全保存。令牌仅显示一次。
    ...
    ### 其他资源- [Dynatrace Events API v2 Documentation](https://docs.dynatrace.com/docs/dynatrace-api/environment-api/events-v2/post-event)
  ...
  <Tab title="Webhook">
    ...
    **先决条件**
    ...
    在LangSmith中配置Webhook之前，请确保您的接收端点：
    ...
    **可选字段**
    ...
    - **请求正文模板**：自定义发送到端点的 JSON 有效负载
      - 默认：LangSmith 发送定义的有效负载以及附加到有效负载的以下附加键值对：
        ...
        - `alert_rule_url`：LangSmith中警报规则的直接链接。- `runs_url`：指向LangSmith 中触发警报的运行的直接链接。
        - `triggered_metric_value`：触发阈值时的指标值。
        - `triggered_threshold`：触发警报的阈值。
        - `timestamp`：触发警报的时间戳。

    <Info>
    LangSmith不对请求体进行模板替换。上面的自动填充字段将作为顶级键与您配置的正文一起合并到传出的 JSON 中。像 `{alert_rule_name}` 这样的占位符语法会逐字发送到接收服务。仅当接收器本身可以从传入的 JSON 中提取字段（例如，Power Automate Workflow、AWS Lambda 或自定义 HTTP 处理程序）时，它才会解析为实际值。
    </Info>
    ...
    **发送测试警报不会验证下游响应。** UI 报告 **您的配置工作正常并且测试通知已发送**，即使接收端点返回错误（例如，400 或 422 拒绝）。始终在接收方验证接收，检查端点的日志或目标平台的消息历史记录，而不是仅仅依赖LangSmith成功消息。
    ...
    <Accordion title="Configure Microsoft Teams notifications via webhook">
      ...
      **先决条件**
      ...
      - 访问具有添加工作流程权限的 Microsoft Teams 工作区。
      - 用于设置警报的LangSmith项目。

      **第 1 步：在 Teams 中创建工作流程**

      1. 在 Microsoft Teams 中，导航到您想要接收警报的频道。
      2. 单击通道名称旁边的 **...**（更多选项）菜单。
      3. 选择**工作流程**。
      ...
      **步骤 2：在 Power Automate 中自定义消息（可选）**
      ...
      默认工作流程将原始 JSON 正文作为卡片发布。要格式化警报详细信息，请在 Power Automate 中编辑流程：1. 打开[Power Automate portal](https://make.powerautomate.com)并编辑您创建的工作流程。
      2. 单击 **聊天或频道中的明信片** 操作。
      ...
      **参考实现**
      ...
      有关将 LangSmith webhook 有效负载（阈值警报、运行规则和通用事件）转换为格式化 Teams 自适应卡的工作示例，请参阅 [langsmith-teams-webhook](https://github.com/langchain-samples/langsmith-teams-webhook) 示例存储库。该示例作为 Teams 工作流 URL 前面的小型 Python 服务运行，这避免了自定义 Power Automate 流本身。
    </Accordion>
    ...
    <Accordion title="Configure Google Chat notifications via webhook (requires middleware)">
      ...
      **第 1 步：部署处理程序**
      ...
      - `LANGSMITH_SHARED_SECRET`：您选择的秘密字符串（用于验证来自LangSmith的传入请求）。

      **步骤2：在LangSmith中配置webhook警报**
