...
通过在检查注释队列中的运行时编写断言，捕获自由格式的验收标准作为数据集示例。
...
2. 在侧面板中，找到 **反馈** 下面的 **断言** 部分。

3. 单击 **+ 添加** 创建断言行。

4. 输入总结声明的**键**（例如，`must_cite_source`、`must_not_invent_url`）和描述该声明的一句话**注释**。

   关键是形式自由。 `must_` / `must_not_` 前缀只是一个命名约定；朗史密斯并没有特别对待他们。

5. 对您要捕获的每个条件重复步骤 3 和 4。
   ...
   <img alt="Annotation queue run editor with assertions added in the side panel and the Outputs panel showing a read-only preview of those assertions." />您可以随时继续编辑运行的**输入**，例如在保存示例之前优化提示。当任何断言保留时，**输出**面板保持锁定到断言预览。

6. 单击侧面板页脚中的 **添加到数据集和下一步**（键盘快捷键：在 macOS 上按 <kbd>⌘ Enter</kbd> 或在其他地方按 <kbd>Ctrl Enter</kbd>）。 LangSmith 将当前运行添加到队列的 [default dataset](/langsmith/annotation-queues#basic-details) 中，或者如果未配置默认值，则提示您选择一个。然后队列会将您移至下一次运行。
...
* **[Partial-credit scoring](/langsmith/multiple-scores)**：返回一个数字分数（例如，0.0 到 1.0 之间）而不是布尔值来按比例评分，并对满足部分（但不是全部）要求的输出给予“部分学分”。
