...
<Note>
  审核日志可在 [**Enterprise** plans](/langsmith/pricing-plans) 上找到。如果您有兴趣升级到企业版，[contact our sales team](https://www.langchain.com/contact-sales)。
</Note>

LangSmith 审核日志提供组织内执行的管理和配置操作的防篡改记录。他们帮助您回答以下问题：
...
审核日志适用于运行 Helm Chart **0.12.33** 或更高版本的 [self-hosted](/langsmith/self-hosted) LangSmith 实例。随着时间的推移，各个操作的覆盖范围不断扩大 - 请参阅[Self-hosted version availability](#self-hosted-version-availability)，了解引入每个操作的图表版本。
...
* **为所有组织启用：** 将以下环境变量添加到 `values.yaml` 中的 `commonEnv`：
  ...
  <Note>
    此环境变量对个人组织没有影响。
  </Note>有关自托管版本的更多详细信息，请参阅 [self-hosted changelog](/langsmith/self-hosted-changelog)。

## 通过API查询审计日志

使用 `GET /api/v1/audit-logs` 端点 ([API reference](/langsmith/smith-api/audit-logs/get-audit-logs)) 检索审核日志事件。结果遵循 [OCSF API Activity](https://schema.ocsf.io/1.7.0/classes/api_activity) 模式。
...
| `api.operation` | LangSmith 操作名称（例如，`create_api_key`、`delete_workspace`）。有关所有值，请参阅[tracked operations reference](#tracked-operations-reference)。 || `status` | `Success`、`Failure` 或 `Unknown`。                                                                                                                          |
...
| **沙箱代理配置文件** | `create_sandbox_proxy_profile`、`update_sandbox_proxy_profile`、`delete_sandbox_proxy_profile` || **游乐场设置** | `create_playground_settings`、`update_playground_settings`、`delete_playground_settings` |
...
<AccordionGroup>
  ...
  <Accordion title="Are read operations logged?">
    审核日志目前主要集中在写操作。将来可能会添加对更多读取操作的支持。
  </Accordion>
  ...
  <Accordion title="What is OCSF?">
    [Open Cybersecurity Schema Framework (OCSF)](https://schema.ocsf.io/) 是安全事件数据的开放标准。 LangSmith 将审核日志事件作为 OCSF v1.7.0 API 活动（类 6003）对象返回。
  </Accordion>
</AccordionGroup>
...
<div>
  ...
  <Callout icon="edit">
    [Edit this page on GitHub](https://github.com/langchain-ai/docs/edit/main/src/langsmith/audit-logs.mdx) 或 [file an issue](https://github.com/langchain-ai/docs/issues/new/choose)。
  </Callout>
</div>