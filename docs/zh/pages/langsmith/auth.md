...
LangSmith提供了灵活的认证和授权系统，可以与大多数认证方案集成。
...
3. 如果无效则引发 [HTTP exception](https://reference.langchain.com/python/langgraph-sdk/auth/exceptions/HTTPException) 或 AssertionError
...
* 通过 [⟦T17⟧](https://reference.langchain.com/python/langgraph-sdk/auth/types/AuthContext) 发送给您的授权处理人员
* 通过 `config["configuration"]["langgraph_auth_user"]` 进行申请

<Accordion title="Supported Parameters">
  [⟦T19⟧](https://reference.langchain.com/python/langgraph-sdk/auth/Auth/authenticate) 处理程序可以按名称接受以下任何参数：
...
该对象包含有关当前用户的信息，包括从 [⟦T26⟧](https://reference.langchain.com/python/langgraph-sdk/auth/Auth/authenticate) 处理程序返回的任何自定义字段。
...
身份验证后，LangGraph 调用[⟦T27⟧](https://reference.langchain.com/python/langgraph-sdk/auth/Auth) 处理程序来控制对特定资源（例如线程、助手、cron）的访问。这些处理程序可以：1. 通过直接改变`value["metadata"]`字典来添加创建资源时要保存的元数据。请参阅 [supported actions table](#supported-actions) 了解每个操作的值可以采用的类型列表。
...
有关更多信息，请参阅参考文献[⟦T51⟧](https://reference.langchain.com/python/langgraph-sdk/auth/Auth)(Auth)。
...
### 基于权限的访问此模式允许您根据**权限**控制访问。如果您希望某些角色对资源具有更广泛或更严格的访问权限，那么它会很有用。
...
| | `@auth.on.store.list_namespaces` |列出命名空间 | `Auth.types.on.store.list_namespaces.value` |存储授权不同于线程和助手。处理程序必须重写 `value` 中的可变 `namespace` 字段以限定每个用户的数据范围，而不是返回元数据过滤器。有关演练，请参阅 [Isolate store per user](/langsmith/store-auth)。
