...
是的。使用 [Bring your own VPC (BYOVPC)](/langsmith/byoc-byovpc)，您可以提供 VPC 和子网。您可以使用[reference Terraform module](https://github.com/langchain-ai/terraform/tree/main/modules/byoc/aws/byovpc)创建VPC，也可以使用现有的满足网络要求的VPC。

要从客户角色中删除 VPC 创建权限，请在 [⟦T1⟧ Terraform module](https://github.com/langchain-ai/terraform/tree/main/modules/byoc/aws/langsmith-byoc-role) 中设置 `allow_vpc_creation_permissions = false`，并在 `vpc_ids` 中包含您的 VPC ID。
</Accordion>

<Accordion title="Can I deploy multiple data planes in the same VPC?">
不可以。目前不支持在同一 VPC 中部署多个数据平面。为每个数据平面使用单独的 VPC。
</Accordion>

<Accordion title="Can I move an existing data plane into my own VPC?">
不可以。不支持将现有数据平面移动到不同的 VPC，也不支持在创建后更改其 VPC 和子网配置。
</Accordion>

<Accordion title="Can I install BYOC into an existing EKS cluster?">
不，并且没有计划支持它。 LangChain 代表您操作集群，包括升级和修补，专用集群使得可靠管理成为可能。
</Accordion><Accordion title="Can I run my own workloads in the LangSmith VPC or cluster?">
...
LangChain 提供外部 ID。导航到 **设置 > 数据平面**，然后使用 **数据平面** 标题旁边的复制按钮进行复制。应用 `langsmith-byoc-role` Terraform 模块时使用此值。角色的信任策略必须在其 `ExternalId` 条件下使用此值。
</Accordion>

<Accordion title="How long does provisioning take?">
数据平面的端到端配置大约需要 60 到 90 分钟。由于 AWS 资源配置不一致，配置时间可能会有所不同。
</Accordion><Accordion title="What happens if provisioning fails?">
数据平面转换到`Provisioning Failed`。失败通常是由于缺少跨账户角色的权限造成的，或者是由于服务控制策略 (SCP) 阻止了LangChain 需要执行的操作造成的。

完全按照发布的方式应用[⟦T6⟧ Terraform module](https://github.com/langchain-ai/terraform/tree/main/modules/byoc/aws/langsmith-byoc-role)，无需任何编辑或更改。修改的模块是缺少权限的常见原因。

联系LangChain团队。更正权限后，配置会自动协调。
</Accordion>

<Accordion title="What VPC CIDR range should I use?">
私有 RFC 1918 范围介于 `/18` 和 `/16` 之间。如果您计划使用 VPC 对等互连，请选择一个与您想要建立对等互连的 VPC 不重叠的范围。 CIDR 范围在配置后无法更改。
</Accordion>
...
</Accordion><Accordion title="How do I decommission a data plane?">
使用 `allow_delete_permissions = true` 重新应用 [⟦T9⟧ Terraform module](https://github.com/langchain-ai/terraform/tree/main/modules/byoc/aws/langsmith-byoc-role)，这将授予删除 LangSmith 托管资源所需的权限。
...
</Accordion><Accordion title="What access do LangChain employees have to my environment and data?">
...
身份验证、组织和工作区配置、计费和使用元数据、哈希 API 密钥以及静态前端资产。跟踪、提示、数据集、评估器、实验、见解运行、注释队列、代理部署和工作区秘密都位于您的数据平面中。
</Accordion>

<Accordion title="What data leaves my account?">
仅运营数据，并且通过 PrivateLink 而不是公共互联网传输：

- **身份验证请求**：数据平面调用控制平面来验证请求、验证 API 密钥并解析角色和权限。
- **配置**：从控制平面加载的组织和工作区配置。
- **计费和使用元数据**。
- **操作遥测**。
...
对于 LangChain 管理的 VPC，BYOC 支持控制数据平面出口的网络防火墙。请联系LangChain团队启用它。启用后，防火墙默认拒绝所有出站流量。然后，您可以配置允许数据平面到达的目的地的允许列表：

- **域**：完全限定的域名，例如模型提供程序 API 端点或公共 Docker 注册表。
- **CIDR 范围**：IP 范围，例如内部服务或私有子网。

通过 BYOVPC，您可以管理自己的防火墙和出口控制。您的网络必须继续满足 [BYOVPC network requirements](/langsmith/byoc-byovpc#meet-the-network-requirements)。
</Accordion>
...
通过 AWS PrivateLink。管理路径仅公开集群的 Kubernetes API 服务器，LangChain 用于安装和协调 LangSmith 组件。
...
</Accordion><Accordion title="Where does the control plane run if my data plane is outside the US?">
...
## 操作<Accordion title="Who is responsible for what in a BYOC deployment?">
...
## 特性和迁移<Accordion title="Which LangSmith features work on BYOC?">
