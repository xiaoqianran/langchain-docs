...
使用 LangSmith 中的聊天来分析跟踪、线程、提示和评估。
...
<img alt="LangSmith Chat icon" /> 聊天出现在 [LangSmith UI](https://smith.langchain.com?utm_source=docs\&utm_medium=cta\&utm_campaign=langsmith-signup\&utm_content=langsmith-chat) 内以下位置的右下角：

<br /><br />**可观察性和调试：**

* [Projects](#projects)：浏览和过滤整个项目的运行。
* [Trace pages](#trace-pages)：分析各个运行和执行跟踪。
* [Thread views](#thread-views)：了解对话线程和用户交互。

**及时工程：**

* [Playground](#playground)：编辑优化提示。
...
* [Evaluators](#evaluators)：在人工智能辅助下构建和完善评估器。
...
在**注释队列**中，聊天可以帮助您在做出注释决策之前分析运行。无论您是单独查看运行还是成对比较，Chat 都可以提供有关运行行为、错误和执行模式的见解，以便为您的评分提供信息。
...
在 **Evaluators** 构建器中，Chat 可帮助您编写和完善评估器逻辑。聊天可以生成评估器代码，提出改进建议，并帮助您根据示例测试评估器。
...
<CardGroup>
  ...
  </Card><Card title="Threads" icon="messages" href="/langsmith/threads">
    了解 LangSmith 中的线程如何工作
  </Card>
