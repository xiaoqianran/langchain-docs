...
当您全局设置环境变量`LANGSMITH_TRACING=true`时，跟踪会自动发送到LangSmith。本指南向您展示如何有选择地禁用或自定义特定请求的跟踪。

当您需要执行以下操作时，请使用条件跟踪：

- **遵守数据保留政策**：出于合规性或隐私原因，某些客户可能需要零数据保留。
...
1. **`ls.configure(enabled=...)`**：全局配置（设置全局跟踪行为）。
...
## 根据请求自定义跟踪配置您还可以动态自定义跟踪设置，例如将跟踪路由到不同的项目或添加特定于请求的元数据。
