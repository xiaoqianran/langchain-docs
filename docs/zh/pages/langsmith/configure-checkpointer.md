...
<Tabs>
  ...
  **捆绑的 MongoDB**（开发和测试）：
  ...
  <Tab title="Cloud">
  在 `langgraph.json` 中将 `backend` 设置为 `"mongo"`，然后将 `LS_MONGODB_URI` 添加为 LangSmith UI 的部署设置中的环境变量。
...
- [Persistence concepts](/oss/python/langgraph/persistence) 于 LangGraph
