...
通过为特定跟踪类型配置自定义预览路径，自定义运行表的输入和输出列中显示的内容。

默认情况下，LangSmith 使用启发式方法来确定在 **运行** 表的 **输入** 和 **输出** 列中显示的内容。但是，您可以通过为特定跟踪类型配置自定义预览路径来准确自定义这些列中显示的内容。
...
1. 导航到项目中的跟踪。
2. 选择**运行**选项卡。
3. 找到运行表右上角的格式图标 <Icon icon="adjustments-horizontal" />。
4. 在 **配置输入和输出预览** 侧窗口中，从下拉列表中选择跟踪名称。

当您选择跟踪名称时，LangSmith 会加载成功的跟踪示例并将其结构呈现为可扩展树。树中的每个节点代表数据中的一个字段，显示：* 字段名称（例如，`messages` 用于 LLM 对话历史记录、`output`、`metadata`）。
* 数组索引（例如，最后一项的 \[0]、\[1]、\[-1]）。
...
1. 选择 **输入** 或 **输出** 选项卡。然后，要么：
   ...
   * 下拉菜单可直接指定应在预览中显示的输入数据的路径。
   * 示例跟踪数据结构的交互式树视图，您可以浏览并选择要显示的确切字段。

   要选择一个字段：

   1. 通过单击箭头图标 (▶) 浏览树以展开或折叠嵌套对象和数组。
   2. 单击要在预览中显示的字段旁边的复选框。所选路径出现在树前面的文本输入中。
...
2. 展开`[1]`（第二条消息，即用户消息）。
3. 单击**内容**旁边的复选框。
4. 输入栏显示：`messages[1].content`。

或者，对最后一条消息使用负索引：1. 展开**消息**。
2. 展开`[-1]`。
3. 单击**内容**。
