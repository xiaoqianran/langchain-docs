...
将 Context Hub 提交事件发送到外部 HTTPS 端点并验证 LangSmith 是否签署了每个请求。
...
2. 单击“**添加 Webhook**”。
3. 输入可公开访问的 HTTPS URL。
4. （可选）添加自定义请求标头，例如 `Authorization` 标头。
5. 单击“**添加 Webhook**”。
6. 复制生成的签名密钥并安全存储。
...
* **滚动签名秘密**：生成并揭示新的签名秘密。 LangSmith 会立即使用新的秘密来进行未来的交付，并且之前的秘密将不再起作用。更新每个验证 webhook 的消费者。
* **删除 webhook**：阻止端点从工作区接收未来的 Context Hub 提交事件。

## 送货

LangSmith 为每个事件发送一个 JSON `POST` 请求。自定义标头无法覆盖 LangSmith 在应用自定义标头后设置的 `Content-Type` 或 `X-LangSmith-Signature`。|物业 |价值|
| ------------------- | ------------------------------------------------------------------------------------ |
|方法| `POST` |
|网址 |可公开访问的 HTTPS 端点 |
...
重试包含字节相同的请求主体并保留事件`id`。在产生下游影响之前，通过`id`删除重复事件。## 验证签名
...
外部 `id`、`type`、`created` 和 `data` 信封被冻结。事件类型上的 `.v1` 后缀版本为 `data.commit` 架构。
...
| `created_by` |字符串 |创建提交的 LangSmith 用户 ID。不可用时省略。          |
| `url` |字符串 | LangSmith UI 中提交的深层链接。                                  |
| `files_changed` |数组|提交中包含文件更改。每个条目包含`path`和`action`。 |

### `data.commit.files_changed`每个条目都总结了一条更改的路径。它不包含文件内容。
...
对 `data.commit` 的重大更改使用了新的事件类型后缀，例如 `.v2`。忽略未知类型而不是尝试将它们解析为 v1，并允许未知字段，以便兼容的添加不会破坏您的处理程序。
