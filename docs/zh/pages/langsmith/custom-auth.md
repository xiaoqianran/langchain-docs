...
本指南向您展示如何向 LangSmith 应用程序添加自定义身份验证。此页面上的步骤适用于 [cloud](/langsmith/cloud) 和 [self-hosted](/langsmith/self-hosted) 部署。它不适用于在您自己的自定义服务器中单独使用[LangGraph open source library](/oss/python/langgraph/overview)。
...
1、实施认证：
   ...
   <Note>
     如果没有自定义的 `@auth.authenticate` 处理程序，LangGraph 只能看到 API 密钥所有者（通常是开发人员），因此请求的范围不限于单个最终用户。要传播自定义令牌，您必须实现自己的处理程序。
   </Note>
