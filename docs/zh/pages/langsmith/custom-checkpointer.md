...
在代理部署中将内置 Postgres 检查点替换为自定义 BaseCheckpointSaver 实现。

将代理部署到 LangSmith 时，服务器提供内置的 Postgres 支持的检查点，用于处理图形运行之间的状态持久性。您可以将其替换为您自己的 [BaseCheckpointSaver](https://reference.langchain.com/python/langgraph/checkpoints/#langgraph.checkpoint.base.BaseCheckpointSaver) 实现，以使用不同的存储后端。
...
大多数开源检查点实现尚未实现代理服务器所需的所有操作。在配置检查点之前，请根据一致性测试套件对其进行验证以确保兼容性。
...
## 能力服务器在启动时检查您的检查指针的**基本**（必需）和**扩展**（可选）功能。如果缺少扩展功能，服务器将使用回退或禁用相应的功能。
...
## 部署您可以将此应用程序按原样部署到 LangSmith 或您的自托管平台。
