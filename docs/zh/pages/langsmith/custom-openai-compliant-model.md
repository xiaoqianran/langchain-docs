...
1. 在 **提示** 标题下，选择型号名称旁边的齿轮 <Icon icon="settings" iconType="solid" /> 图标。
1. 在 **模型配置** 选项卡中，从下拉列表中选择要编辑的模型。
1. 对于 **提供商** 下拉列表，选择 **OpenAI 兼容端点**。
1. 将您的 OpenAI 兼容端点添加到 **基本 URL** 输入。示例请参见[Base URL format](#base-url-format)。
