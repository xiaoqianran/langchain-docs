...
配置和管理 LangSmith 批量导出的 S3 兼容导出目标。
...
* **访问密钥**：S3 存储桶的访问密钥。
* **密钥**：S3 存储桶的密钥。
* **在前缀中包含存储桶**（可选）：是否将存储桶名称作为路径前缀的一部分包含在内。默认为`true`。当使用存储桶名称已存在于端点 URL 中的虚拟托管样式端点时，设置为 `false`。
...
1. 在您的云提供商中配置新凭证，使其具有对目标存储桶和前缀的写入权限。
