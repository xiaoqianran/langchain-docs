...
为了更好地支持 LLM 工作流程，LangSmith 支持一些不同的预定义预构建类型。这些模式由 LangSmith API 公开托管，并且可以使用 [JSON Schema references](https://json-schema.org/understanding-json-schema/structuring#dollarref) 在数据集模式中定义。可用模式表如下所示|类型 | JSON 架构参考链接 |用途 |
| -------- | ------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------ |
...
|工具| [https://api.smith.langchain.com/public/schemas/v1/tooldef.json](https://api.smith.langchain.com/public/schemas/v1/tooldef.json) |可用于聊天模型进行函数调用的工具定义，以 OpenAI 的 JSON Schema 启发函数格式定义。 |
