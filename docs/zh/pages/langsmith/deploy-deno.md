...
</Steps>或者，使用内置的 `deno deploy` CLI (Deno 2.x)。 [⟦T15⟧](https://github.com/langchain-ai/deployment-cookbook/blob/main/js-deno/deno.json)中的`deploy`块设置`org`/`app`。将它们更改为您自己的（或传递 `--org`/`--app` 标志，这会覆盖它们）。
...
## 所需的 API 端点该应用程序在`/api/threads/...`下公开代理流协议。路由处理程序位于 `server/routes.ts` 中，并镜像 `js-next/app/api/threads/` 中的 Next.js 处理程序。
...
- **`server/threads.ts`**：LangGraph SDK 有线格式中支持检查指针的 `getState` / `updateState` / `getHistory` 帮助程序。
...
- `main.ts`：Deno 部署入口点（`Deno.serve` + Hono）。
