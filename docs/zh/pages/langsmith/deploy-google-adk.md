...
使用deployments-wrap-sdk包将Google代理开发工具包（ADK）代理部署到LangSmith代理服务器。
...
`deployments-wrap-sdk` 提供了一个瘦包装器，可将配置好的 ADK `Runner` 转换为与 LangGraph 兼容的图，因此您可以部署 ADK 代理，而无需自己编写 [Functional API](/oss/python/langgraph/functional-api) 胶水。包装纸：

* 将 ADK 会话桥接到代理服务器的[checkpoint persistence](/langsmith/agent-server#persistence)，因此会话状态可以在重新启动后继续存在并在运行期间恢复。
* 通过 LangGraph 的流管道转发 ADK 令牌事件，因此部分令牌显示在 [⟦T14⟧](/langsmith/streaming) 和 [LangSmith Studio](/langsmith/studio) 中。
* 当`LANGSMITH_TRACING`设置时，自动为ADK启用[LangSmith tracing](/langsmith/trace-with-google-adk)。

## 先决条件

* Python 3.11+
* [LangGraph CLI](/langsmith/cli) 用于本地开发和部署
* LangSmith API密钥，参考[Create an account and API key](https://docs.langchain.com/langsmith/create-account-api-key)
* 如果您使用 Gemini 型号，则需要 Google AI API 密钥，请参阅[Google AI Studio](https://aistudio.google.com/api-keys)
...
对于真正的代理，删除`before_model_callback`并直接配置模型。例如，通过设置 `model="gemini-2.5-flash"` 和 `GOOGLE_API_KEY` 设置来使用 Gemini，或者通过 ADK 的 LiteLLM 适配器（`google.adk.models.lite_llm.LiteLlm`，可通过 `google-adk[extensions]` 获得）使用 Claude/OpenAI。
...
* **每轮多条新消息**：只有`messages`中的最后一项被视为新用户消息。对话历史记录是根据 ADK 会话状态重建的，而不是根据 LangGraph 消息列表重建的。
...
* **原生 LangGraph 中断**：包装器不公开 LangGraph 的 `interrupt` 或 `Command(resume=...)` 机制。基于 `LongRunningFunctionTool` 构建的人机交互流程遵循 ADK 自己的模式：该工具返回诸如 `pending_approval` 之类的状态，代理回复，后续轮流解决待处理的呼叫。
...
代理在本地运行后，使用 `langgraph deploy` 将其部署到 LangSmith：
...
## 启用跟踪每当启用 LangSmith 跟踪时，`wrap()` 都会自动调用 `langsmith.integrations.google_adk.configure_google_adk()`，因此您所需要做的就是在部署上设置环境变量：
...
包装已配置的 `google.adk.runners.Runner` 并返回 LangGraph `Pregel` 图，该图可以从模块导出并由代理服务器提供服务。
...
2. 包装器将前一个会话（如果有）从 LangGraph 检查点加载到`LangsmithSessionService`，然后要求运行器处理最新消息。
