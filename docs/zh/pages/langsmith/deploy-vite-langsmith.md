...
**LangSmith 部署** 在 LangSmith 的托管代理服务器上运行 LangGraph 图。在这个例子中：

- `agent/` 包含深层代理图、子代理、中间件和工具。
- `langgraph.json` 告诉 LangGraph CLI 要提供和部署哪个图。
- `src/` 包含 React 聊天 UI。
- UI 通过 LangGraph SDK 和 `@langchain/react` 与代理服务器 API 进行对话。
...
在本地开发过程中，`pnpm run dev`同时启动LangGraph开发服务器和Vite应用程序。在生产环境中，LangSmith 托管代理，静态主机服务于 Vite 构建的 UI。
...
部署后，在LangSmith中打开部署并复制其**API URL**。它应该看起来像：
...
浏览器客户端在与远程部署通信时重用`LANGSMITH_API_KEY`。<Warning>
该演示将 `LANGSMITH_API_KEY` 暴露给浏览器捆绑包，以便 UI 可以直接调用 LangSmith 部署。这对于本地测试来说很方便，但不利于生产安全。对于真正的应用程序，通过您自己的后端代理请求并将密钥保留在服务器端。
</Warning>
...
3. 设置这些环境变量：
   ...
   - `LANGSMITH_API_KEY`：演示客户端使用的LangSmith API 密钥。
...
部署之前在 Netlify 中添加这些变量：- `VITE_AGENT_API_URL`：LangSmith 部署根 URL。
- `LANGSMITH_API_KEY`：演示客户端使用的LangSmith API 密钥。
...
````agent/index.ts`导出LangGraph本地服务和LangSmith部署的图。本地`MemorySaver`检查指针仅由`langgraph dev`使用。 LangSmith 部署将其替换为生产中由 Postgres 支持的持久存储，无需更改代码。
...
- 动作：`langgraphjs deploy` 至 LangSmith。
...
此示例是 **LangSmith 部署**。您可以在代码中定义图形，将其部署到LangGraph代理服务器，并从标准代理服务器API进行流式传输。

托管 Deep Agents 版本将使用代码优先 `managed-deepagents` 包和 `mda` CLI。在该模型中，LangChain 托管代理运行时，而项目将其代理定义、工具、中间件、指令、技能和可选沙箱保留在源代码中。

当您想要完全控制图形代码、中间件、自定义代码工具、检查点行为或标准代理服务器 API 时，请使用此示例。当您需要更少的运行时基础设施并且可以通过托管 Deep Agents 代码优先合约进行创作时，请使用托管 Deep Agents。 */}
