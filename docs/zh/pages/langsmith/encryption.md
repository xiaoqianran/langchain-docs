...
<!-- langchain-docs: Add encryption at rest | https://docs.langchain.com/langsmith/encryption -->

# 添加静态加密

代理服务器支持检查点数据和元数据的静态加密。您可以选择使用单个密钥的基本加密或针对高级用例的自定义加密。
...
对于使用单个静态密钥的简单加密，请设置 `LANGGRAPH_AES_KEY` 环境变量。 LangGraph 将使用 AES 自动加密检查点 blob。

1. 将 `pycryptodome` 添加到 `langgraph.json` 中的依赖项：
   ```json
   {
...
要同时加密特定 JSON 字段，请将 `LANGGRAPH_AES_JSON_KEYS` 设置为要加密的以逗号分隔的密钥列表：
...
这些密钥无论出现在线程、助手、运行、cron 和存储数据中的任何位置都会被加密。<Warning>
无法搜索或过滤加密字段。
</Warning>
...
<Note>
需要代理服务器版本 0.6.22+ 和 Python SDK 版本 `langgraph-sdk>=0.3.1`。
</Note>
...
<Note>
如果您要从基本加密迁移，请保留 `LANGGRAPH_AES_KEY` 配置。自定义加密处理新写入，同时现有 AES 加密数据仍然可读。
</Note>
...
**JSON 处理程序** (`@encryption.encrypt.json` / `@encryption.decrypt.json`) 递归地应用于以下字段：
...
从经过身份验证的用户中获取加密上下文，而不是显式传递 `X-Encryption-Context`：
...
<Note>
加密上下文以明文形式存储。解密时，它会自动恢复——调用者在读取时不需要传递标头。
</Note>
