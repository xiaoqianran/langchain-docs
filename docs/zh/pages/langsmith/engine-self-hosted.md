...
在自托管 LangSmith 中，引擎的编排（包括其检测、修复和验证循环）作为 LangSmith 的一部分在您的 VPC 内运行。模型工作无法完全在您的 VPC 中运行：引擎将其所需的内容发送到LangSmith Intelligence (LSI)，这是LangChain 管理的零数据保留 (ZDR) 服务。本页解释了这对您的数据意味着什么。
...
网关主机是[⟦T4⟧](/langsmith/deploy-self-hosted-full-platform#allow-egress-to-langsmith-intelligence)。 LSI 将请求路由到 LangChain 的 AWS 环境中的 AWS Bedrock。

#### 使用 AWS PrivateLink 连接在配置 PrivateLink 之前，请完成[Enable Engine](/langsmith/deploy-self-hosted-full-platform#enable-engine)，包括其 Helm 和 egress 配置。
...
<Steps>
  ...
  <Step title="Verify private connectivity">
    ...
    确认结果包含分配给端点网络接口的专用 IP 地址。然后[enable Engine](/langsmith/deploy-self-hosted-full-platform#enable-engine)，开始分析，并确认成功完成。如果分析未完成，请检查引擎安装和出口配置。
  </Step>
</Steps>
