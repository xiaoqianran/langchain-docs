...
<Callout>
有关 Enterprise [pricing](/langsmith/pricing-plans) 或入门的问题，请[contact our sales team](https://www.langchain.com/contact-sales)。
</Callout>

## 托管选项

选择如何托管LangSmith以满足您的基础设施和数据驻留要求。

<CardGroup cols={3}>
  <Card title="Cloud" icon="cloud" href="/langsmith/cloud">
    在 LangSmith 的托管云中托管 LangSmith，并具有美国或欧盟数据驻留。
  </Card>
  <Card title="Hybrid" icon="topology-complex" href="/langsmith/hybrid">
    在LangSmith的云中运行控制平面，在您自己的VPC中运行数据平面，以实现完全数据隔离。
  </Card>
...
管理用户并自动化整个组织的配置。<CardGroup cols={2}>
...
查看 LangSmith 的安全状况和合规性认证。<CardGroup cols={2}>
  <Card title="Shared responsibility model" icon="shield-check" href="/langsmith/shared-responsibility-model">
    查看 LangChain 和您的组织之间共同承担的安全责任。 LangSmith 拥有 SOC 2 Type II、HIPAA 和 GDPR 认证。
  </Card>
