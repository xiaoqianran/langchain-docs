...
<!-- langchain-docs: Evaluate a complex agent | https://docs.langchain.com/langsmith/evaluate-complex-agent -->
...
为了简单起见，在本演示中，我们将通过删除相应的数据库记录来实现退款。我们将跳过实施用户身份验证和其他生产安全措施。

代理的逻辑将被构造为两个独立的子图（一个用于查找，一个用于退款），以及一个将请求路由到适当子图的父图。
...
现在让我们定义我们的图表。我们将使用具有三个主要路径的简单架构：

1. 从对话中提取客户和购买信息

2. 将请求路由到三个路径之一：

   * 退款途径：如果我们有足够的购买详细信息（发票 ID 或发票行 ID）来处理退款
...
您可以在此处查看这些结果：[LangSmith link](https://smith.langchain.com/public/708d08f4-300e-4c75-9677-c6b71b0d28c9/d)。

### 轨迹评估器

随着代理变得更加复杂，它们有更多潜在的故障点。与使用简单的通过/失败评估不同，通常更好的做法是使用当代理采取一些正确步骤时可以给予部分信用的评估，即使它没有达到正确的最终答案。

这就是轨迹评估的用武之地。轨迹评估：

1. 将代理采取的实际步骤顺序与预期顺序进行比较
2. 根据正确完成预期步骤的数量计算分数
...
```现在我们可以进行评估了。我们的评估器假设我们的目标函数返回一个“轨迹”键，因此让我们定义一个执行此操作的目标函数。我们需要使用[LangGraph's streaming capabilities](/oss/python/langgraph/streaming)来记录轨迹。
...
您可以在此处查看这些结果：[LangSmith link](https://smith.langchain.com/public/f133dae2-8a88-43a0-9bfd-ab45bfa3920b/d)。
