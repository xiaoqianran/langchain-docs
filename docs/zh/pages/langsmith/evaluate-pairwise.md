...
自定义求值器函数必须具有特定的参数名称。它们可以采用以下参数的任意子集：* `inputs: dict`: A dictionary of the inputs corresponding to a single example in a dataset.
...
For most use cases you'll only need `inputs`, `outputs`, and `reference_outputs` / `referenceOutputs`.仅当您需要应用程序实际输入和输出之外的一些额外跟踪或示例元数据时，`runs` 和 `example` 才有用。
