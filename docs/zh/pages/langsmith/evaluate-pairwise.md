...
目前仅支持Python* `list[int | float | bool]`：包含两项的分数列表。假定该列表与 `runs` / `outputs` 评估器参数具有相同的顺序。评估器函数名称用于反馈键。
...
![Pairwise Comparison View](/langsmith/images/pairwise-comparison-view.png)您可以通过单击表标题中的“拇指向上”/“拇指向下”按钮来筛选第一个实验更好的运行，反之亦然：
