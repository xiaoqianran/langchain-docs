...
每周限制法学硕士在评估员上的支出，并在组织范围内默认或按评估员覆盖，以保持评估员成本可预测。
...
<Tip>
  LangSmith 还提供 [per-trace and per-model cost tracking](/langsmith/cost-tracking) 和 [tracing usage limits](/langsmith/administration-overview#usage-limits) 用于成本控制。
</Tip>

<Warning>
  设置支出限制适用于 OpenAI、Anthropic 和 Gemini 模型。支出限制仅针对在 LangSmith 中具有 [pricing configured](/langsmith/cost-tracking#create-a-new-or-modify-an-existing-model-price-entry) 的受支持模型上执行。在依赖限制之前验证模型定价。一旦设置限制，不支持的模型就不能在评估器中使用。
</Warning>
...
## 执法如何运作LangSmith 在每次评估程序运行完成后记录支出，然后对从周一上午 12 点（世界标准时间）到当前时刻的支出进行汇总。当总数达到有效限制时，LangSmith 会暂停附加项目或数据集上的评估器。在最终确定之前，运行中的运行可能会使总数略高于上限，因此支出可能会短暂超出一小部分。

代理和跟踪不受影响。只有评估者才会停止生成分数，直到支出限额重置或限额为[manually increased](#override-the-default-for-an-attached-project-or-dataset)。

## 花费视图和控件|查看 |在哪里可以找到它 |谁可以看到或更改它 |
| -------------------------------------------------------------------------------------------------- | --------------------------------------------------- | ------------------------------------------------------------------------ |
...
* **支出状态**：以下之一：
  ...
  * **N 个限制命中**：评估器在其附加的一个或多个项目或数据集中已达到其限制。该数字反映了有多少个已暂停。
  * **无限制**：未设置限制。
...
1. 打开组织**设置**并导航至**使用配置**。
2. 对于**评估者支出限额**，输入美元金额。单位是`/ week`。留空表示没有限制。
3. 单击**保存**。如果未设置组织默认值，则附加项目和数据集不受限制，除非配置了自定义覆盖。清除默认值会删除当前继承它的每个附加项目或数据集的上限。
...
2. 单击右上角的 **编辑评估器** 图标。
3. 在**来源**下，选择特定项目或数据集。
4. 滚动经过**过滤器**和**采样率**，然后展开**高级**。
5. 在 **支出限额** 字段中，设置自定义美元金额。单位是`/ week`。
6. **保存**评估器配置。
...
* LangSmith 在该项目或数据集的新运行中停止运行评估器。
...
**创建评估器时遇到问题**：设置限制后，评估器必须使用受支持的模型（OpenAI、Anthropic 或 Gemini），并在 [Model pricing](/langsmith/cost-tracking#create-a-new-or-modify-an-existing-model-price-entry) 中输入定价。**LangSmith 支出与我的 LLM 提供商发票不匹配**：LangSmith 根据 [Model pricing](/langsmith/cost-tracking#create-a-new-or-modify-an-existing-model-price-entry) 中配置的每个模型费率计算支出，而不是根据提供商的账单。如果您的提供商应用了您尚未添加到 LangSmith 的折扣、定制合同或型号变体，则预计会出现差异。
