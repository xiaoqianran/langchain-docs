...
LangSmith Fleet 要素是构成代理基础的核心功能。它们包括工具、渠道、内存、子代理和批准。
...
* **共享**：控制谁可以使用代理，可选择私人、工作空间或特定人员。参见[Change access to the agent](/langsmith/fleet/manage-agent-settings#change-access-to-the-agent)。
...
* **计划**：定期运行您的代理。参见[Schedules](/langsmith/fleet/schedules)。
...
当您的代理人停下来请求批准时，您有两种选择：<CardGroup>
...
队列使用情况以 LangChain 计算单元 (LCU) 来衡量。 LCU 使用情况基于代理执行的 [model](#models) 工作，包括所选层及其处理和生成的内容量。<Note>
  新的 [model tiers](#models) 和 LCU 定价适用于自 **2026 年 7 月 15 日**开始的新机队使用情况。在此日期之前已经使用 Fleet 的组织将保留其当前设置，并于 **2026 年 10 月 1 日**过渡到新模型。如果您使用自定义模型，请联系您的 LangChain 客户团队了解转换事宜。
</Note>
...
Fleet 提供三个托管层。随着新模型的推出，每层背后的模型可能会随着时间的推移而发生变化，因此您可以根据需要完成的工作而不是特定的提供商或模型进行选择。|等级 |最适合 | <Tooltip>相对成本</Tooltip> |
| -------- | --------------------------------------------------------------------------- | -------------------------------- |
| **快** |研究、总结和起草等日常任务 |低|
...
在托管舰队模型选择器中，自定义模型不与 Fast、Pro 和 Max 一起使用。 LangChain 管理托管层的模型提供者访问，因此您不需要自己的模型提供者 API 密钥。如果企业部署需要自定义模型，请联系您的 LangChain 客户团队或[reach out to sales](https://www.langchain.com/contact-sales)。

## 自我更新

代理可以自我更新：他们可以添加新工具、删除不需要的工具或调整指令。但是，代理无法更改其名称、描述或启动它们的频道。

## 技能技能是一种捆绑功能并在上下文不普遍相关的情况下提供更具体信息的方法。

使用技能可以帮助：

* 通过仅提供与当前任务相关的上下文来节省令牌的使用。
* 防止座席在系统提示中出现过多上下文，这可能会导致幻觉和错误响应。
...
2. 右键单击​​要跟踪的线程并选择“**查看跟踪**”。

欲了解更多信息，请参阅[LangSmith Observability](/langsmith/observability)。

<Note>
  Fleet 跟踪所有代理运行并将其存储在 LangSmith 中。 LLM 提供商不会保留您的数据。在 LangSmith Cloud 上，跟踪数据默认存储为 14 天的保留期。
</Note>
