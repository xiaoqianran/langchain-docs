...
- **模型提供者密钥**：默认情况下，Fleet 使用由 LangChain 管理的模型，不需要模型提供者 API 密钥。仅当您使用 [custom models](/langsmith/fleet/essentials#custom-models) 时，才需要 OpenAI 或 Anthropic API 密钥。设置后，代理图会从工作区机密加载此密钥以进行推理。
- **特定于队列的机密**：以 `FLEET_` 为前缀的机密优先于队列内的工作区机密。这样，您可以更好地跟踪 Fleet 与使用相同秘密的 LangSmith 其他部分的使用情况。如果您同时拥有 `OPENAI_API_KEY` 和 `FLEET_OPENAI_API_KEY`，则将使用 `FLEET_OPENAI_API_KEY` 秘密。
...
1. 选择 **添加密钥** 并输入密钥**名称**（例如，`OPENAI_API_KEY` 或 `ANTHROPIC_API_KEY`）以及您的密钥作为**值**。
...
**使用情况**页面使工作区管理员可以了解队列支出以及为代理和用户设置支出限制的能力。此页面仅对具有 `fleet:read-admin-config` 权限的用户可见。
...
#### 支出限制行为- 对支出限制的更改可能需要几分钟才能在所有正在运行的代理中传播。
