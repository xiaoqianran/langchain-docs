...
- 确定哪些用户或[API keys](/langsmith/create-account-api-key#api-keys)消耗最多的痕迹或运行最多的代理。
- 分析一段时间内的使用模式。
- 导出使用数据以供内部报告。
...
4. 使用控件来：
   - 选择时间范围（过去 7 天、30 天、3 个月、6 个月、1 年或自定义）
...
- `end_time` 向上舍入到下一个 UTC 午夜（已经在午夜时无操作）。
- 与请求窗口重叠的任何一天均完整包含在内。

因此，从 `2026-01-01T12:00:00Z` 到 `2026-01-02T12:00:00Z` 的 24 小时窗口会返回完整的 1 月 1 日和 1 月 2 日存储桶的使用情况。

### 跨步每个响应中的`stride`字段指示用于聚合的时间桶大小，根据请求的时间范围计算。每天是最少的。子日窗犹斗一日。
...
`kind=langsmith_deployments` 与 `group_by=trace_tier` 结合返回 `400 Bad Request`。保留层仅适用于跟踪。
...
<Note>
**LangSmith 部署使用情况** 与跟踪使用情况分开获取，并且可用于部署使用情况的完整保留窗口。对于自托管实例，部署使用端点是可选的。通过以下方式启用它：

```env
REMOTE_METRICS_ROLLUP_ENABLED=true
```

或者升级到默认启用它的LangSmith版本（请参阅[self-hosted changelog](/langsmith/self-hosted-changelog)）。
</Note>
...
|项目 ID / 名称 |始终（在 `group_by=project` 时填充）|
