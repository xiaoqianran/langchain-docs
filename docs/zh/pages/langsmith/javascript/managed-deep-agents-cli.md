...
传递给 `mda evals init` 的任务名称可以包含 ASCII 字母、数字、`_` 和 `-`。`mda evals compile` 标志：
...
有关 Harbor 任务创作、可选脚手架、凭据和运行试验的信息，请参阅 [Evals](/langsmith/javascript/managed-deep-agents-evals)。
