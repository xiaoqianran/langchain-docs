...
<!-- langchain-docs: Manage connections | https://docs.langchain.com/langsmith/javascript/managed-deep-agents-connections -->
...
`connections.get(...)` 选择所有者。它不会创造一个。请求没有自己凭证的所有者无法运行 `agent`，并为 `user` 提出 [⟦T55⟧ interrupt](#handle-the-authorization-interrupt)。存储在不同所有者的同一 slug 下的凭证不满足查找。代理拥有的凭据属于项目的部署，因此在至少一次成功[⟦T57⟧](/langsmith/javascript/managed-deep-agents-deploy)之后创建它们。部署仅读取其拥有的凭证。要从第二个部署中解析相同的 slug，请从该项目根目录再次运行 `mda connections create <slug>`：在现有 slug 上，该命令会附加新的代理拥有的凭据，而不是失败。
...
| SDK 客户端或自定义前端 |项目[identity declaration](/langsmith/javascript/managed-deep-agents-identity)验证的身份。 |

默认身份声明验证LangSmith API 密钥。该密钥对调用客户端（而不是个人）进行身份验证，因此提供该密钥的每个调用者都会解析为相同的身份。要为每个登录者提供自己的凭据，请声明 [Supabase identity](/langsmith/javascript/managed-deep-agents-identity#authenticate-end-users-with-supabase)。
...
| **通用 OAuth** |您注册自己的 OAuth 应用程序 (BYOT)，例如使用 GitHub 或 Google。 |目录中的 `--oauth <service>`，或自定义提供商的 `--authorize-url` 和 `--token-url` |每个呼叫者或带有 [⟦T65⟧](#authorize-an-agent-owned-oauth-account) 的座席 |
| **MCP OAuth** | MCP 服务器通告 OAuth 并自动注册客户端。 | `--mcp <url>`，或者当项目已经声明用户拥有的 MCP 服务器时单独使用 slug |每个呼叫者或具有 [⟦T67⟧](#authorize-an-agent-owned-oauth-account) 的代理 |
...
请参阅 [Custom tools](/langsmith/javascript/managed-deep-agents-tools) 将工具添加到代理。
...
大多数提供商会拒绝没有 `scope` 参数的授权请求。通过`--scope`进行手动注册，或者在目录涵盖提供商时使用`--oauth <service>`。### 授权代理拥有的 OAuth 帐户

当提供商发布自己的应用程序凭证（例如 Slack 机器人令牌、GitHub 应用程序安装令牌或 Notion 内部集成令牌）时，最好将该凭证存储为 [agent-owned secret](#create-an-opaque-secret)。应用程序凭证的范围仅限于应用程序，可以自行撤销，并且不依赖于任何人的帐户。
...
在`tools/mcp.ts`中添加服务器并引用连接slug。
...
有关更多信息，请参阅[Connect to MCP servers](/langsmith/javascript/managed-deep-agents-mcp-connectors)。
...
如果服务器无法自动注册客户端，CLI 会这样说明并指向您通用 OAuth（`--oauth` 或手动端点）。`mda deploy` 将相同的推论应用于丢失的用户拥有的 MCP 连接。它保持现有连接不变，通过 OAuth 发现创建每个唯一的缺失 MCP 连接，并在发现或注册不可用时在部署之前失败。
...
### 中断负载对于[⟦T170⟧](/oss/javascript/langchain/frontend/human-in-the-loop#setting-up-usestream) (`@langchain/react`、`@langchain/vue`、`@langchain/svelte`) 或`injectStream` (`@langchain/angular`)，待处理中断位于`stream.interrupt`。凭证门有效负载为`stream.interrupt.value`。
...
呈现此中断的前端是一个 Web 应用程序，因此无论代理是用 Python 还是 TypeScript 编写的，这些示例都是 TypeScript。检测`stream.interrupt`上的凭证门有效负载，渲染连接卡，然后在存储每个授权后使用`stream.respond`恢复：
...
一般中断生命周期（`stream.interrupt`、恢复、检查点）请参见[Human-in-the-loop](/oss/javascript/langchain/frontend/human-in-the-loop)。

### 处理 OAuth 授权 (`kind: "oauth2"`)
...
### 内置 Slack 处理当代理通过 Slack 通道运行时，Slack 会呈现包含 HTTPS `connect_url` 的 OAuth 条目。调用者打开链接，完成同意，Slack 恢复运行，因此没有人需要打开 LangSmith 来连接服务。
...
使用 `list` 或 `get` 检查连接元数据。 `list` 显示工作区中的每个连接，而不仅仅是该项目使用的连接：
...
连接是 Managed Deep Agents 公共测试版的一部分。当前版本存在以下差距：- **用户拥有的 API 密钥**：用户拥有的连接持有 OAuth 授权。 Slack 和 Studio 不会收集每个调用者的 API 密钥，并且 CLI 不会为其创建一个空槽。请改用代理拥有的密钥，或在自定义前端中收集密钥，如下节所述。
