...
欲了解更多信息，请参阅[Project structure](/langsmith/javascript/managed-deep-agents-project-structure)。<Note>
...
`mda deploy` 将值作为托管部署机密转发。声明没有此值的后端身份的项目将无法在任何构建工作之前进行部署预检，因为部署将拒绝带有 401 的每个请求。有关 `.env` 值如何到达部署，请参阅 [Deploy](/langsmith/javascript/managed-deep-agents-deploy#secrets-and-environment-files)。
