...
`mda init` 搭建沙箱声明。托管 Deep Agents 仅当 `sandbox/` 目录存在时才启用沙箱。
...
在沙箱中使用[Connections](/langsmith/javascript/managed-deep-agents-connections)来验证CLI命令和API请求。
...
## 选择烘焙底料在没有烘焙基础的情况下，LangSmith的默认沙盒模板是起点。要从其他内容开始，请准确设置其中之一：
...
将`GHCR_TOKEN`放入项目`.env`或流程环境中。烘焙后，托管Deep Agents 不会将该值转发到正在运行的代理服务器。

## 代理如何使用沙箱

该代理使用内置文件系统工具，例如[⟦T46⟧](/oss/javascript/deepagents/tools#built-in-harness-tools)、[⟦T47⟧](/oss/javascript/deepagents/tools#built-in-harness-tools)、[⟦T48⟧](/oss/javascript/deepagents/tools#built-in-harness-tools)、[⟦T49⟧](/oss/javascript/deepagents/tools#built-in-harness-tools)、[⟦T50⟧](/oss/javascript/deepagents/tools#built-in-harness-tools)、[⟦T51⟧](/oss/javascript/deepagents/tools#built-in-harness-tools)和[⟦T52⟧](/oss/javascript/deepagents/tools#built-in-harness-tools)，并使用[⟦T53⟧](/oss/javascript/deepagents/tools#built-in-harness-tools)运行shell命令。使用[instructions](/langsmith/javascript/managed-deep-agents-instructions)指定代理应该在哪里工作以及不能修改什么。

## 从代码中读取和写入沙箱文件

[Authored tools](/langsmith/javascript/managed-deep-agents-tools)和[middleware](/langsmith/javascript/managed-deep-agents-middleware)通过`runtime.backend`到达沙箱文件系统。当您自己的代码需要文件时使用它，而不是提示代理为您获取文件。<Note>
...
| ---| ---|
| `ls(path)` |列出一个目录。 |
| `read(filePath, offset, limit)` |读取文本，默认2000行。 |
| `readRaw(filePath)` |读取没有行格式的文件。 |
| `write(filePath, content)` |写入文本，替换任何现有文件。 |
| `edit(filePath, oldString, newString, replaceAll)` |就地替换子字符串。 |
| `delete(filePath)` |删除一个文件。 |
| `grep(pattern, path, glob, maxCount)` |搜索文件内容。 |
| `glob(pattern, path)` |匹配路径。 |
| `execute(command)` |运行外壳命令。 |
| `uploadFiles(files)` |编写 `[path, content]` 对中的 `Uint8Array` 内容。 |
| `downloadFiles(paths)` |将每条路径读取为`Uint8Array`。 |

每个方法都会返回一个承诺。


参数和返回类型来自 Deep Agents 后端合约。参见[Backends](/oss/javascript/deepagents/backends)。
...
`runtime.backend` 仅涵盖沙盒。它没有到 [Context Hub](/langsmith/javascript/managed-deep-agents-context-hub) 的路由，因此无法通过它到达 [skills](/langsmith/javascript/managed-deep-agents-skills)、[instructions](/langsmith/javascript/managed-deep-agents-instructions) 和 [memory](/langsmith/javascript/managed-deep-agents-memory)。



如果没有沙箱，`runtime.backend`就是`undefined`。在每次调用之前对其进行保护，因为项目可以在工具发布后删除`sandbox/`。




`delete`、`uploadFiles` 和 `downloadFiles` 取决于已安装的后端，并在未实现它们时抛出异常。根`glob`返回错误而不是配置沙箱。
