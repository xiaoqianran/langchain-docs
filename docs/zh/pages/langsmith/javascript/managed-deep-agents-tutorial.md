...
本教程从[quickstart](/langsmith/javascript/managed-deep-agents-quickstart)继续。使用您在此处创建的 `research-assistant` 项目，以及您的模型、说明、搜索工具和有效的 `mda dev` 设置。
...
<Steps>
  ...
  <Step title="(Optional) Add a custom tool">
    ...
    <Tabs>
      ...
      <Tab title="Tavily">
        打开`agent.ts`。保持您的快速入门`model`值：<CodeGroup>
