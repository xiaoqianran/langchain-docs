...
本教程一次培养研究助理的一项能力。首先完成[quickstart](/langsmith/javascript/managed-deep-agents-quickstart)来搭建项目，添加API密钥，然后在本地运行`mda dev`。然后添加搜索工具，使用持久内存，按每日计划运行代理，并将其部署到 LangSmith。
...
<Steps>
  ...
  <Step title="Add a search tool">
    ...
    有关创作工具的更多信息，请参阅[Custom tools](/langsmith/javascript/managed-deep-agents-tools)。
  </Step>
  ...
  <Step title="Schedule a daily digest">
    ...
    部署上线后，`mda deploy` 将此计划协调到 LangSmith cron 作业中。有关线程行为和约束，请参阅[Schedules](/langsmith/javascript/managed-deep-agents-schedules)。
  </Step>
  ...
  <Step title="Deploy the agent">
    ...
    mda deploy .
    ```成功后，CLI 将打印部署仪表板 URL。部署将指令同步到 Context Hub、上传已编译的项目并协调每日计划。有关部署标志和故障排除，请参阅 [Deploy an agent](/langsmith/javascript/managed-deep-agents-deploy) 和 [CLI reference](/langsmith/javascript/managed-deep-agents-cli#deploy-projects)。
  </Step>
