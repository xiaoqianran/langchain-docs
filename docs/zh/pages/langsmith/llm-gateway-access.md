...
默认情况下，所有组织的内容跟踪均处于关闭状态。当跟踪内容打开时，网关代理的调用将被跟踪到与调用者的 API 密钥关联的 [workspace](/langsmith/administration-overview#workspaces) 中名为 `gateway` 的项目，以及具有方案 `gateway-<short_api_key>-<api_key_id>` 的 API 密钥特定​​项目。

使用 [RBAC](/langsmith/rbac) 和 [ABAC](/langsmith/abac) 控制对这些跟踪项目的访问

### 跟踪元数据

网关代理调用与直接 LLM 调用的区别在于它们所在的项目以及附加到其跨度的元数据：- **网关项目：** 所有网关流量都写入每个工作区中的中央网关项目，以及名为 `gateway` 的固定 LangSmith 项目，并在 `gateway-<short_api_key>-<api_key_id>` 处具有每个 API 密钥副本，以实现 UI 隔离。按项目（或按`langsmith.metadata.gateway.*` span 属性的存在）过滤以查找网关代理的调用。
...
使用数据保留策略，您可以禁止将跟踪内容（请求和响应正文）写入跟踪。这将适用于通过网关发出的与策略范围匹配的任何跟踪。

## LangSmith 引擎集成当治理策略触发时（例如达到支出限制、检测到并编辑 PII 或捕获秘密时），该事件将作为元数据记录在跟踪上。这些违反政策的行为在 LangSmith 引擎中表现为问题。
