...
| `OPENAI_API_KEY` |开放人工智能 |
| `VERTEX_SERVICE_ACCOUNT_JSON` |谷歌顶点人工智能 |仅添加您的组织使用的提供商。如果用户尝试调用尚未添加密钥的提供商，网关将返回错误。
...
1. 转至 **设置 → 成员/角色**。
2. 创建新的工作区角色。
3. 至少授予`gateway:invoke`和`workspaces:read`。
4. 将需要网关访问权限的用户分配给此角色。
...
网关策略管理需要`organization:manage`权限。转到 **设置 → 网关 → LLM 网关** 以创建治理策略。您可以配置：
