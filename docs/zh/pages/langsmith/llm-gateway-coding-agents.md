...
配置 Claude Code、Codex、Gemini CLI 和 Deep Agents Code 以通过 LLM 网关路由 LLM 呼叫。
...
Claude Code 可以使用标准的 Anthropic Messages 格式，而 Codex 和 Deep Agents Code 可以使用标准的 OpenAI 兼容格式。 Gemini CLI 使用 Google 的本机 API 并需要 [direct model access](/langsmith/llm-gateway-direct-model-access)。
...
claude
```克劳德代码将 `/v1/messages` 附加到 `ANTHROPIC_BASE_URL`。网关使用 `anthropic/` 前缀来解析工作区的 Anthropic 提供者机密。

<Warning>
  配置网关时，Claude Desktop 插件会中断。尚不支持付费计划（Plus、Max）的 Claude 用户。
</Warning>
...
<Warning>
  配置网关时 Codex Desktop 插件会中断。 TOML 配置强制通过网关进行身份验证，因此 OpenAI 不再直接处理插件身份验证。
</Warning>
...
使用具有标准端点的 OpenAI 兼容客户端，然后通过 `openai` 集成传递托管模型 slug：
...
1. 每个客户端的标准网关基本 URL。
2. 每个用户或团队的工作区范围[LangSmith API key](/langsmith/create-account-api-key)，具体取决于您的策略粒度。
3. 为每个编码代理批准的型号 ID。
4. Codex `config.toml`（如果您的组织使用 Codex）。

提供商 API 密钥集中保存在 LangSmith 工作区机密中。 Gateway Credits 模型不需要提供商 API 密钥。
...
1. 呼叫成功，座席收到响应。
2. 跟踪将出现在 LangSmith 工作区的`gateway` 或`gateway-<short_api_key>-<api_key_id>` 跟踪项目中。如果调用失败并显示 `403`，请检查您的 API 密钥的角色是否包含 `gateway:invoke` 和 `workspaces:read`。如果自带密钥调用失败并显示 `400` 提及缺少提供程序密钥，请要求组织管理员将提供程序的密钥添加到工作区机密中。
...
* [Spend policies](/langsmith/llm-gateway-spend-policies)：对开发者LLM使用设置成本限制。
