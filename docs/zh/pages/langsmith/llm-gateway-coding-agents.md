...
- 您有一个工作区范围的 [LangSmith API key](/langsmith/create-account-api-key) 以及 `gateway:invoke` 和 `workspaces:read` [permissions](/langsmith/organization-workspace-operations)。
...
将 `ANTHROPIC_API_KEY` 设置为您的 LangSmith API 密钥。 Claude Code 从您的 shell 环境或 `~/.claude/settings.json` 中的 `env` 块读取这些变量。
...
将 `ANTHROPIC_BASE_URL` 设置为 Anthropic 格式的网关端点。网关从端点推断 `anthropic/` 提供商前缀：
...
如果调用失败并显示 `403`，请检查您的 API 密钥的角色是否包含 `gateway:invoke` 和 `workspaces:read`。如果自带密钥调用失败并出现 `400` 提及缺少提供程序密钥，请要求您的组织管理员将提供程序的密钥添加到工作区机密中。
