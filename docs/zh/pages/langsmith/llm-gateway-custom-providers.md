...
两条路由查找相同的配置，解析相同的密钥，并代理到相同的上游 URL；它们的区别仅在于是否强制使用型号名称。
...
按名称调用保存的配置（以下示例中的`my-custom-openai-endpoint`和`my-anthropic-endpoint`）。
...
</CodeGroup>网关将请求体的`model`字段原样转发给上游。
