...
- **PII 编辑** 默认处于关闭状态，并为选定的组织启用。如果没有它，PII 选项在创建和编辑表单中不可用，并且 API 会拒绝配置为检测 PII 的任何策略。

请联系您的客户团队以更改任一权利。

## 数据保留通过 LLM 网关的每个调用都会追溯到 LangSmith。 **跟踪内容** 切换控制请求和响应正文是否存储在这些跟踪中。它对所有组织都是关闭的，并且数据策略会针对策略匹配的请求将其打开。当跟踪内容被禁用时，网关仍然记录元数据，例如令牌使用情况、延迟、状态、模型信息和策略评估结果。当它打开时，会记录相同的元数据以及通话内容。
...
当数据策略启用检测时，网关会在出站请求到达 LLM 提供商之前对其进行扫描。在将请求转发到上游之前，检测到的值将替换为占位符，因此提供者只能看到经过编辑的有效负载。对于成功的响应，网关会在将响应返回给调用者之前将占位符恢复为原始值。内容仅在发送至法学硕士时才进行编辑。一旦响应返回，该内容就会在返回给调用者之前重新水化。如果启用跟踪内容捕获，网关跟踪将反映提供者所看到的内容，包括经过编辑的占位符。网关外部的应用程序跟踪仍然可以捕获从该响应接收到的恢复值。

**PII** 和 **Secrets** 切换是独立的。在新策略中，PII 检测从选择的每条规则（对于具有权利的组织）开始，并且秘密检测开始。

### PII 检测

**个人身份信息 (PII)** 切换涵盖姓名、电子邮件、美国电话号码、地址和美国 SSN。它下面有六个规则，每个规则都可以单独选择，并根据网关检测它们的方式进行分组。
...
当检测到 PII 或秘密时，发送到提供者的请求和LangSmith 跟踪中的内容将被替换为占位符。例如：
...
LangSmith 中的跟踪显示了编辑版本以及指示发生了编辑以及检测到哪些类别的元数据。
