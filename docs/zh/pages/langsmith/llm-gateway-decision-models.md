...
将 `LANGSMITH_API_KEY` 设置为工作区范围的 LangSmith API 密钥。 `typesafe/` 前缀通过工作区的 TypeSafe 提供程序密钥（而不是托管的 SemIf 模型）路由请求。调用网关时，请勿将 TypeSafe API 密钥作为 SDK 的 `api_key` 或 `apiKey` 传递。对于 SDK 使用不带 `/v1` 的网关基本 URL：
