...
政策立即生效。速率限制策略还可以在自定义请求标头上携带条件，因此来自单个主题的流量会按标头值拆分为单独的限制。使用此功能可以在一个 API 密钥下为您的每个最终客户提供自己的吞吐量限额。欲了解更多信息，请参阅[Per-customer policies](/langsmith/llm-gateway-header-policies)。
...
- [Data policy](/langsmith/llm-gateway-data-policy)：添加数据策略。
