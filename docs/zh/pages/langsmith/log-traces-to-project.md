...
使用环境变量或 SDK 将 LangSmith 跟踪到指定项目而不是默认项目。

本页介绍如何控制 LangSmith 发送跟踪的位置：
...
LangSmith 使用 [*project*](/langsmith/observability-concepts#projects) 的概念对迹线进行分组。如果未指定，项目将设置为 `default`。
...
如果指定的项目不存在，LangSmith 将在引入第一个跟踪时自动创建它。
...
如果您需要根据运行时配置将跟踪动态路由到不同的 LangSmith [workspaces](/langsmith/administration-overview#workspaces)（例如，将不同的用户或租户路由到单独的工作区），则方法因语言而异：
...
此方法对于您希望在工作区级别按客户、环境或团队隔离跟踪的多租户应用程序非常有用。它适用于任何与 LangSmith 兼容的跟踪，包括 LangChain、OpenAI 和用 `@traceable` 修饰的自定义函数。
...
当 [deploying agents](/langsmith/deployment) 到 LangSmith 时，您可以使用图形生命周期上下文管理器覆盖发送跟踪的默认工作区。当您想要根据通过 `config` 参数传递的运行时配置将跟踪从已部署的代理路由到不同的工作区时，这非常有用。
...
<Note>
  使用跨工作区跟踪进行部署时，请确保您的服务密钥或 PAT 具有所有目标工作区的必要权限。我们建议使用多工作区服务密钥进行生产部署。对于 LangSmith 部署，您必须添加一个可跨工作空间访问环境变量的服务密钥（例如，`LS_CROSS_WORKSPACE_KEY`），以覆盖部署生成的默认服务密钥。
</Note>## 使用副本将跟踪写入多个目标
...
<Warning>
  您不能将 `LANGSMITH_RUNS_ENDPOINTS` 与 `LANGSMITH_ENDPOINT` 一起使用。如果同时设置两者，LangSmith 会引发错误。仅使用一个来配置您的端点。
</Warning>
...
<Warning>
  身份验证不会在分布式跟踪中传播。当跟踪跨越多个服务时，LangSmith 会自动将副本 `project_name` 和 `updates` 转发到下游服务，但不会转发 API 密钥或凭证。每个服务必须为副本目标配置自己的凭据。
</Warning>
...
您可以在运行时决定给定调用是否将跟踪发送到 LangSmith、OpenTelemetry (OTel) 后端或同时发送到两者，而无需重新部署或修改应用程序逻辑。当您想要在每个环境甚至每个请求的可观察性后端之间切换并在运行时做出决定时，这非常有用。使用 `tracing_mode` 构造函数参数或 `LANGSMITH_TRACING_MODE` 环境变量设置跟踪模式。两者都接受相同的价值观；显式 `tracing_mode` 参数始终优先于环境变量：
...
<CodeGroup>
  ...
  await jokeDynamic();
  ```
</CodeGroup>每个 `Client` 上的 `tracing_mode` 确定该副本的导出路径。在 Python 中，`"hybrid"` 模式处理单个副本中的两个目标。在 TypeScript 中，“发送到两者”的情况使用两个单独的副本，每个客户端一个，因为没有 `"hybrid"` 模式。由于每个副本独立解析其自己的客户端，因此您还可以在单​​个`tracing_context`内混合模式，例如，保留一个副本发送到 LangSmith，同时通过第二个副本将相同的跟踪转发到 OTel 收集器。
