...
LangSmith 的 API 支持通过 API 密钥以编程方式访问 UI 中的所有可用操作，只有 [User-only endpoints](#user-only-endpoints) 中指出的少数例外情况。
...
<Warning>
  使用 `X-Tenant-Id` 标头指定要定位的工作区。如果标头不存在，则操作将默认到最初创建密钥的工作区（如果它不在组织范围内）。
...
* `/sso/email-verification/send`（仅限云）：该端点与[SAML SSO](/langsmith/user-management)相关
