...
如果您已经从 LangSmith 将 `LANGSMITH_API_KEY` 设置为当前工作区的 api 密钥，则可以跳过此步骤。
...
## 推送提示要创建新提示或更新现有提示，您可以使用 `push prompt` 方法。
...
要拉取提示，您可以使用 `pull prompt` 方法，该方法将提示作为 langchain `PromptTemplate` 返回。要拉出**私人提示**，您不需要指定所有者句柄（尽管可以，如果您有一组）。

要从 LangChain Hub 中提取**公共提示**，您需要指定提示作者的句柄。
...
<CodeGroup>
  ...
  对于拉取提示，如果您使用 Node.js 或支持动态导入的环境，我们建议使用 `langchain/hub/node` 入口点，因为它会自动处理与提示配置关联的模型的反序列化。

  如果您处于非 Node 环境中，非 OpenAI 模型不支持“includeModel”，您应该使用基本 `langchain/hub` 入口点。
</Note>

## 提示缓存

LangSmith SDK 包括内置的内存中提示缓存。启用后，LangSmith 将在内存中缓存拉取的提示，从而减少频繁使用的提示的延迟和 API 调用。缓存使用在所有客户端之间共享的全局单例实例，并在进程的生命周期内持续存在。它实现了“重新验证时失效”模式，确保您的应用程序始终获得快速响应，同时在后台保持提示最新。
...
您可以配置所有客户端默认使用的全局提示缓存。当您想要在整个应用程序中自定义缓存行为时，这非常有用：
...
要禁用特定客户端的缓存，请传递 `disable_prompt_cache=True`。您还可以全局配置最大大小为零：<CodeGroup>
  ```python Python theme={"theme":{"light":"catppuccin-latte","dark":"catppuccin-mocha"}}
  from langsmith import Client
...
</Note>## 在没有 LangChain 的情况下使用提示

如果您想将提示存储在 LangSmith 中但直接通过模型提供商的 API 使用它们，则可以使用我们的转换方法。这些会将您的提示转换为 OpenAI 或 Anthropic API 所需的有效负载。
