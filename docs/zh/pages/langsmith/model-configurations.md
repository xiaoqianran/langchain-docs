...
模型配置定义了LangSmith功能在调用AI提供商时使用的模型和参数。单个共享配置库跨越整个[workspace](/langsmith/administration-overview#workspaces)，因此您创建的任何配置都可以在以下功能中使用，而无需重复：
...
配置还可以携带 [OAuth client credentials](#oauth-client-credentials)，因此 LangSmith 在请求时针对您的 IdP 铸造短期持有者令牌，而不是使用静态 API 密钥。
...
1. 导航到 **设置** > **模型配置**。
1. 在 **功能访问** 表中，找到您要配置的功能。
1. 单击“**启用的提供程序**”并打开或关闭该功能的提供程序。
1. 单击“**可用型号**”并选择用户可以选择的型号。
1. 使用**默认模型**下拉列表设置用户打开该功能时预选的模型。
...
1. 导航至**设置**下的[Model providers](https://smith.langchain.com/settings/model-providers)页面。
1. 关闭某个提供程序以在整个组织中禁用它，或打开以重新启用它。
...
创建组织范围的配置需要 [Organization Admin](/langsmith/rbac#organization-admin) 角色：

1. 导航到组织级别的 **设置** > **模型配置**。
1. 在“**配置**”下，单击“**+ 创建**”。
1. 选择**提供商**和**型号**，并填写其余字段。
1. 单击“**保存**”。
...
1. 在 **配置** 表中找到组织范围的配置。
1. 在 **功能访问** 表中，启用应显示的每个功能的配置。

### 限制

使用组织范围的配置时，请记住以下限制：

- **范围是不可变的。** 保存为组织范围的配置无法更改为工作空间范围，并且工作空间范围的配置无法提升为组织范围。如果需要更改范围，请创建新配置。
- **工作区优先处理名称冲突。** 当工作区配置和组织范围的配置共享相同名称时，工作区版本优先。
- **机密存储位置。** 在 LangChain 云（非 BYOC）上，组织范围配置的机密存储在 LangChain 云控制平面上。在 BYOC 部署中，您可以使用数据平面切换器配置模型机密的写入位置。

## 配置**配置**表是工作区的命名模型配置的共享库。您在LangSmith（包括来自[Playground](/langsmith/managing-model-configurations)）中创建的配置显示在此处，您可以在所有功能中重复使用它们。
...
1. 在“**配置**”下，单击“**+ 创建**”。
1. 选择**提供商**和**型号**。
1. 输入 **API 密钥名称** - 工作区中存储提供程序 API 密钥的密钥的名称。
1. 根据需要调整参数。参数分为以下几个部分：
...
1. 在 **配置** 表中，单击配置旁边的溢出菜单 <Icon icon="dots-vertical" iconType="regular" />。
1. 选择**编辑**。
1. 更新配置并单击**保存**。### 删除配置
...
当请求针对支持 OAuth 的配置运行时，LangSmith 通过内部代理创建一个承载，缓存结果，并在每个出站 LLM 调用上标记该承载，直到缓存的令牌过期。
...
在配置上启用 OAuth 时，LangSmith 不会提示输入该配置的工作区密钥，因为代理在请求时提供凭据。
