...
本节将向您展示如何使用 [Helm Chart](https://github.com/langchain-ai/helm/tree/main/charts/langsmith-observability) 部署 LangSmith 的端到端可观测性堆栈。
...
安装可观察性 helm 图表后，您需要在 *LangSmith* helm 配置文件中设置以下值，以启用日志和跟踪的收集。
...
然后端口转发到端口 3000 处的 `langsmith-observability-grafana` 容器，并以 `localhost:3000` 打开浏览器。使用用户名 `admin` 和上面密钥中的密码登录 Grafana。进入 Grafana 后，您可以使用 UI 来监控日志、指标和跟踪。 Grafana 还预先打包了一组仪表板，用于监控部署的主要组件。
