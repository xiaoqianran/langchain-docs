...
<Info>
有关LangSmith的RBAC体系、角色定义和权限概念的概述，请参阅[Role-based access control](/langsmith/rbac)。
</Info>
...
- [User-level operations](#user-level-operations)：所有经过身份验证的用户的操作
...
### 组织成员|运营|组织管理员 |组织运营商|组织用户 |组织查看器 |所需权限 |笔记|
|------------|:---------:|:------------:|:--------:|:----------:|--------------------|--------|
...
|邀请会员（批量）| ✓ | ⚠ | ✗ | ✗ | `organization:manage` |组织操作员只能邀请组织用户和组织查看者 |
|添加基本​​授权成员 | ✓ | ⚠ | ✗ | ✗ | `organization:manage` |组织操作员只能添加组织用户和组织查看者 |
...
|创建组织范围的服务密钥（工作空间范围）* | ✓ | ✓ | ⚠ | ✗ | `organization:pats:create` + `workspaces:manage-keys` |
|创建组织范围的服务密钥（组织范围）* | ✓ | ✗ | ✗ | ✗ | `organization:pats:create` + `organization:manage` |
|删除组织范围的服务密钥（工作空间范围）* | ✓ | ✓ | ⚠ | ✗ | `organization:read` + `workspaces:manage-keys` |
|更新服务关键角色 | ✓ | ✓ | ✗ | ✗ | `organization:manage` |
|列出自己的个人访问令牌 (PAT) | ✓ | ✓ | ✓ | ✗ | `organization:read` |
|列出每个成员的个人访问令牌 (PAT) | ✓ | ✓ | ✗ | ✗ | `organization:pats:read` |
|创建个人访问令牌 (PAT) | ✓ | ✓ | ✓ | ✗ | `organization:pats:create` |
|停用、重新激活或删除自己的个人访问令牌 (PAT) | ✓ | ✓ | ✓ | ✗ | `organization:read` |
|停用、重新激活或删除其他成员的个人访问令牌 (PAT) | ✓ | ✓ | ✗ | ✗ | `organization:pats:manage` |<Note>
\* 组织操作员和组织用户只能在其角色授予 `workspaces:manage-keys` 的工作区（例如工作区管理员角色）中创建或删除工作区范围的服务密钥。创建组织范围的服务密钥需要组织管理员角色。
</Note>
...
|获取作业配置见解 | ✓ | ✓ | ✓ | `rules:read` |
|创建见解作业配置 | ✓ | ✓ | ✗ | `rules:create` |
|自动生成见解作业配置 | ✓ | ✓ | ✗ | `rules:create` |
...
根据运行条件触发操作的自动运行规则。|运营|工作区管理员 |工作区编辑器 |工作区查看器 |所需权限 |
|------------|:---------------:|:--------------:|:----------------:|--------------------|
|列出所有运行规则 | ✓ | ✓ | ✓ | `rules:read` |
|创建运行规则 | ✓ | ✓ | ✗ | `rules:create` |
|更新运行规则 | ✓ | ✓ | ✗ | `rules:update` |
...
|下载数据集（OpenAI格式）| ✓ | ✓ | ✓ | `datasets:download` |
|下载数据集（OpenAI微调格式）| ✓ | ✓ | ✓ | `datasets:download` |
...
|运行游乐场实验（流）| ✓ | ⚠ | ✗ | `prompts:read` + `datasets:read` + `projects:create` |
...
<Note>反馈公式运算已弃用，取而代之的是[composite evaluators](/langsmith/composite-evaluators-ui)，并计划于 2026 年 8 月 20 日删除。</Note>|运营|工作区管理员 |工作区编辑器 |工作区查看器 |所需权限 |
|------------|:---------------:|:--------------:|:----------------:|--------------------|
|列出反馈公式（已弃用）| ✓ | ✓ | ✓ | `feedback:read` |
|获取反馈公式（已弃用）| ✓ | ✓ | ✓ | `feedback:read` |
|创建反馈公式（已弃用）| ✓ | ✓ | ✗ | `feedback:create` |
|更新反馈公式（已弃用）| ✓ | ✓ | ✗ | `feedback:update` |
|删除反馈公式（已弃用）| ✓ | ✓ | ✗ | `feedback:delete` |
|查看具体反馈 | ✓ | ✓ | ✓ | `feedback:read` |
|列出反馈 | ✓ | ✓ | ✓ | `feedback:read` |
...
LLM 输出的人工审核队列。|运营|工作区管理员 |工作区编辑器 |工作区查看器 |所需权限 |
|------------|:---------------:|:--------------:|:----------------:|--------------------|
...
|更新在队列中运行 | ✓ | ✓ | ✗ | `annotation-queues:update` |
...
用户创作的迷你 Web 应用程序在 LangSmith 中呈现。参见[Custom Apps](/langsmith/custom-apps)。|运营|工作区管理员 |工作区编辑器 |工作区查看器 |所需权限 |
|------------|:---------------:|:--------------:|:----------------:|--------------------|
...
[LangSmith Deployment](/langsmith/deployment) 配置。|运营|工作区管理员 |工作区编辑器 |工作区查看器 |所需权限 |
|------------|:---------------:|:--------------:|:----------------:|--------------------|
...
|获取批量出口目的地 | ✓ | ✓ | ✓ | `bulk-exports:read` |
|创建批量导出目的地 | ✓ | ✗ | ✗ | `bulk-exports:manage` |
|更新批量导出目的地 | ✓ | ✗ | ✗ | `bulk-exports:manage` |
...
|管理车队支出限额 | ✓ | ✗ | ✗ | `fleet:write-admin-config` |

## 用户级操作

这些操作可供所有经过身份验证的用户使用，不需要特定的工作区或组织权限：- 查看自己的用户个人资料
- 更新自己的用户个人资料
- 列出用户的组织
- 创建新组织
- 列出待处理的工作区邀请
