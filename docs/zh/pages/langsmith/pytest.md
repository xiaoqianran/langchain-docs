...
要在 LangSmith 中跟踪测试，请添加 `@pytest.mark.langsmith` 装饰器。每个修饰的测试用例都将同步到数据集示例。当您运行测试套件时，数据集将被更新，并且将创建一个新实验，每个测试用例都有一个结果。
...
运行此测试将创建/更新一个名为“test\_foo”的示例，输入`{"a": 1, "b": 2}`，参考输出`{"foo": "bar"}`，并跟踪输出`{"foo": "baz"}`的运行。
...
def test_cd(c: int, d: int) -> None:
    ...
    assert result == d
```这将创建/同步一个名为“test\_cd”的示例，输入`{"c": 5}`和参考输出`{"d": 6}`，并运行输出`{"d": 10}`。

## 记录反馈

默认情况下，LangSmith 收集每个测试用例的`pass`反馈键下的通过/失败率。您可以使用 `log_feedback` 添加其他反馈。
...
该装置是动态的（可以读取环境变量、调用函数等）并遵循 pytest 的 `conftest.py` 层次结构，因此它可以按目录划分范围。
...
每个测试套件都会有一个漂亮的表格，当结果上传到 LangSmith 时，该表格会实时更新：
...
## 试运行模式如果您想运行测试而不将结果同步到 LangSmith，您可以在您的环境中设置 `LANGSMITH_TEST_TRACKING=false`。
...
LangSmith 提供了一个 [expect](https://reference.langchain.com/python/langsmith/observability/sdk/expect/) 实用程序来帮助定义对 LLM 输出的期望。例如：
