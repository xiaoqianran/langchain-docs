...
该工具需要三件事：要发布到的 Slack 对话、机器人令牌以及将表单与中断联系起来的相关 ID。- **对话**：`runtime.channel.raw_event` 是启动运行的 Slack 事件。它携带`channel`、`ts`，对于线程消息，还携带`thread_ts`。发布到该线程中，以便答案路由回同一个代理线程。
...
就像注册任何其他[custom tool](/langsmith/python/managed-deep-agents-tools)一样，在代理定义中注册该工具。中断继承了托管检查指针，因此不需要额外的持久性设置。
...
与 `runtime.channel.event` 相同的响应在运行上下文中可用，其中 `type` 设置为 `user_prompt_response`。 Slack 的逐字交互有效负载位于 `runtime.channel.raw_event`。对于在消息内包含输入的表单，用户的条目位于 `raw_event["state"]["values"]`，由块 ID 和操作 ID 键入。
...
## 另请参阅- [Human-in-the-loop](/langsmith/python/managed-deep-agents-tools#human-in-the-loop)：在使用平台渲染的批准卡调用选定工具之前暂停。
- [Connect a Managed Deep Agent to Slack](/langsmith/python/managed-deep-agents-channels-slack)：添加传递表单响应的 Slack 通道。
- [Manage connections](/langsmith/python/managed-deep-agents-connections)：`connections.get(...)` 如何解析代理拥有的凭据。
- [Add custom tools](/langsmith/python/managed-deep-agents-tools)：注册发布表单的工具。
