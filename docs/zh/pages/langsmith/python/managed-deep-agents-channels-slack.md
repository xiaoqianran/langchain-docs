...
<Note>
Slack 中的人机交互请求仅支持 `approve` 和 `reject` [decision types](/langsmith/python/managed-deep-agents-tools#human-in-the-loop)。
</Note>
...
- [Sandboxes](/langsmith/python/managed-deep-agents-sandboxes)：为代理提供文件传输读写的文件系统。
- [Deploy an agent](/langsmith/python/managed-deep-agents-deploy)：配置和部署托管深度代理。
- [CLI reference](/langsmith/python/managed-deep-agents-cli)：查看托管 Deep Agents 命令和标志。
