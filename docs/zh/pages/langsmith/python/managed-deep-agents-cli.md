...
CLI 首先从项目 `.env` 文件中读取这些值，然后从流程环境中读取这些值。如果在交互式终端中找不到密钥，`mda deploy` 会提示输入 LangSmith API 密钥并将其保存到项目 `.env` 文件中。```text .env theme={"theme":{"light":"catppuccin-latte","dark":"catppuccin-mocha"}}
...
传递给`mda evals init`的任务名称可以包含ASCII字母、数字、`_`和`-`。
