...
将远程 MCP 服务器中的工具添加到托管 Deep Agents。

MCP 连接器将工具从远程 [Model Context Protocol (MCP)](/oss/python/deepagents/mcp) 服务器添加到托管深度代理。托管 Deep Agents 创建 MCP 客户端、加载工具并将其添加到代理。
...
* **[Authored tools](/langsmith/python/managed-deep-agents-tools)** 在项目中实现应用逻辑，并通过代理定义传递。
