...
通常，托管深度代理的对话内存范围仅限于线程或会话。持久内存是代理可以在**跨**线程和会话中保留的可选知识。

启用后，持久内存由 [Context Hub](/langsmith/use-the-context-hub) 支持。部署在`/memories/agent/`获取一棵读/写树，由每个调用者共享。默认情况下，托管深度代理**不**具有持久内存。
...
内存不是你的系统提示符。在[instructions](/langsmith/python/managed-deep-agents-instructions)中定义始终在线的行为，在[skills](/langsmith/python/managed-deep-agents-skills)中定义特定于任务的过程；使用内存来获取代理在运行时学到的持久知识。
...
启用内存会在代理文件系统中的 `/memories/agent/` 处挂载一棵 Context Hub 树 `memories/agent`：|路径|使用|
| ------------------------------------------------ | -------------------------------------------------------------------------------------------------- |
...
代理根据提示决定要记住什么。为了使策略明确，请将如下所示的指南添加到`instructions.md`并使其适应您的应用程序：
