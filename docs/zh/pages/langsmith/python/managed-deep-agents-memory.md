...
内存不是你的系统提示符。在[instructions](/langsmith/python/managed-deep-agents-instructions)中定义始终在线的行为，并在[skills](/langsmith/python/managed-deep-agents-skills)中定义特定于任务的过程；使用内存来获取代理在运行时学到的持久知识。
