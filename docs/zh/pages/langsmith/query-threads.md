...
## 列出项目中的所有线程`list_threads` / `listThreads` 获取项目中的所有线程并将它们的运行分组在一起。结果首先按最近的活动排序。
...
| `order` | `"asc" \| "desc"` | `"asc"` |排序顺序。 `"asc"` 返回最旧的优先（按时间顺序）运行。 |
| `select` | `string[]` |所有领域 |返回特定的运行字段，以减少响应大小。 |
