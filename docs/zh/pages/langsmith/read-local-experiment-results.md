...
<Note>
  本页面重点介绍以编程方式处理结果，同时仍将结果上传到 LangSmith。

  如果您想在本地运行评估**而不**向 LangSmith 记录任何内容（用于快速测试或验证），请参阅使用 `upload_results=False` 的 [Run an evaluation locally](/langsmith/local)。
</Note>
...
* `result["run"]`：目标函数的执行。
  ...
  * `result["run"].id`：本次运行的唯一ID。* `result["evaluation_results"]["results"]`：`EvaluationResult` 对象的列表，每个评估者一个。
  * `key`：指标名称（来自评估器的返回值）。
