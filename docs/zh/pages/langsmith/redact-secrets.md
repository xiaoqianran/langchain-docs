...
使用 SDK 匿名器防止 API 密钥、令牌和其他机密出现在 LangSmith 跟踪中。
...
<Info>
  ...
  * TypeScript SDK：0.1.33及以上
</Info>

`create_anonymizer` 函数接受正则表达式模式和替换字符串的列表。将生成的匿名器传递给 [Client](https://reference.langchain.com/python/langsmith/client/Client) 构造函数，它将在到达 LangSmith 之前自动应用于所有运行输入和输出。

以下示例编辑了常见的秘密格式，包括 OpenAI API 密钥、通用不记名令牌和 `sk-` 前缀密钥：
...
<CodeGroup>
  ...
  ```
</CodeGroup>匿名器将输入和输出序列化为 JSON，应用每个正则表达式模式，然后在发送给 LangSmith 之前反序列化结果。默认情况下，它最多遍历 10 层嵌套。要更改此设置，请传递 `max_depth` 参数：
