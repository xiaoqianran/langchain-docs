...
<!-- langchain-docs: Reject Concurrent | https://docs.langchain.com/langsmith/reject-concurrent -->

# 拒绝并发

本指南假设您了解什么是双重短信，您可以在 [double-texting conceptual guide](/langsmith/double-texting) 中了解。

该指南介绍了双短信的 `reject` 选项，该选项通过抛出错误来拒绝图形的新运行，并继续原始运行直到完成。下面是使用 `reject` 选项的快速示例。
