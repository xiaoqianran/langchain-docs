...
自托管 LangSmith 的发布渠道、节奏和版本支持。

自托管的 LangSmith 有两个发布渠道：一个是客户在生产中运行的稳定渠道，另一个是跟踪下一个主要版本的预览渠道。

## 发布渠道

### 稳定

当前普遍可用的主要版本。 LangSmith 推荐使用此通道进行生产。 Stable 每周都会收到补丁版本，其中仅包含关键错误修复和安全补丁。主要版本之间没有新功能、数据迁移或基础设施变化稳定。
...
预览版旨在用于测试和登台环境中的评估。 LangSmith 不建议在生产中运行预览。
...
* **LangSmith 版本** 是图表部署的应用程序版本，在图表中记录为 `appVersion`。|                             |图表版本|朗史密斯版|
| ------------------------ | | -------------- | ----------------- |
|稳定| `0.15.17` | `0.15.24` |
...
* 两个数字独立前进。图表补丁号和 LangSmith 补丁号预计不会匹配。

用于发布列车的`vX`简写（`v15`、`v16`）指的是主要版本，因此`v15`表示图表版本`0.15.Y`。

## 版本支持

LangSmith 支持当前的稳定主要版本（`N`）和之前的稳定主要版本（`N-1`）：
...
* **保留受支持的版本。** LangSmith 建议在每个新的主要版本发布后立即升级到该版本，以便按照建议的节奏进行架构改进。
