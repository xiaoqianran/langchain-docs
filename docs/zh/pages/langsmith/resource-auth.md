...
<!-- langchain-docs: Make conversations private | https://docs.langchain.com/langsmith/resource-auth -->
...
在开始本教程之前，请确保 [bot from the first tutorial](/langsmith/set-up-custom-auth) 运行无错误。
...
```恭喜！您已经构建了一个聊天机器人，每个用户都有自己的私人对话。虽然该系统使用简单的基于令牌的身份验证，但这些授权模式将适用于实现任何真正的身份验证系统。在下一个教程中，您将使用 OAuth2 将测试用户替换为真实用户帐户。
