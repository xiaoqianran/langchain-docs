...
使用此方法使内部/公共 CA 在整个系统范围内受到 LangSmith 的信任（Playground 模型调用和[database/external service connections](/langsmith/self-hosted#storage-services)）。1. 创建一个文件，其中包含 TLS 与数据库和外部服务所需的所有 CA。如果您的部署在没有代理的情况下直接与 `beacon.langchain.com` 通信，请确保包含公共信任的 CA。所有证书都应在此文件中连接起来，中间有一个空行。
   ```
   -----BEGIN CERTIFICATE-----
   <PUBLIC_CA>
   -----END CERTIFICATE-----
