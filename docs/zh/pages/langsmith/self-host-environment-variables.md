...
`extraEnv` 是每个服务的字段，仅向特定服务添加环境变量。每个支持 `extraEnv` 的服务都将其公开在 `<service>.deployment.extraEnv` 下。当变量仅适用于一项服务时，或者需要在 `playground` 或 `aceBackend` 上设置`commonEnv` 达不到的变量时，请使用`extraEnv`。
