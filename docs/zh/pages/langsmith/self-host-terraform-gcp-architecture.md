...
LangSmith 在 GKE 上自托管的平台层、服务、工作负载身份、网络和模块依赖项。

了解 [GCP Terraform modules](https://github.com/langchain-ai/terraform/tree/main/modules/gcp) 的内容以及各部分如何组合在一起，以便您可以在运行 `make apply` 之前调整、保护和自定义 LangSmith 部署。
...
* 模块描述和依赖关系。
* 网络、工作负载身份和流量。
* 附加组件：LangSmith Deployment、Fleet、Insights 和 Polly。
...
LangSmith 在 GCP 上的部署最多分为五个阶段。每个阶段都会在前一个阶段的基础上添加一个功能层。所有层共享相同的 GKE 集群和 `langsmith` 命名空间。

<img alt="LangSmith on GCP deployment stages and service layout" />|舞台|层 |添加了什么 |
| -----| -------------------- | ---------------------------------------------------------------------------------------------------------------------------------- |
| 1 | GCP 基础设施 | VPC、GKE、Cloud SQL、Memorystore、GCS、K8s 引导程序、证书管理器、KEDA、Envoy 网关 |
| 2 |兰史密斯基地|前端、后端、平台后端、队列、ace 后端、clickhouse、游乐场 |
| 3 | LangSmith 部署 |主机后端、侦听器、操作员 + 每个部署的 pod |
| 4 |舰队|独立舰队 API 服务器、独立舰队工具服务器、独立舰队触发服务器、独立舰队队列 |
| 5 |见解+波莉| Clio 分析（ClickHouse 支持）、Polly 评估代理 |<Note>
...
| `postgres` | `infra/modules/postgres/` | Cloud SQL PostgreSQL实例，区域HA备用，私有IP，删除保护 || `redis` | `infra/modules/redis/` | Memorystore Redis STANDARD\_HA 层，VPC 内的私有 IP |
...
## 应用核心服务|服务 |目的|港口|羟丙胺 |工作负载身份 |取决于 |
| ---------------------------- | ---------------------------------------------------------------------- | ---- | ------------------------ | | ----------------- | -------------------------------- |
...
| `langsmith-playground` | LLM提示游乐场UI | 3001 | 3001 1 到 10 |没有 | `backend` || `langsmith-queue` |跟踪摄取工作人员（Redis 到 ClickHouse + GCS）| — | 3 至 10 + 科达 |是的 | Redis、ClickHouse、GCS |
...
| `langsmith-operator` | Kubernetes 运营商。协调 `LangGraphPlatform` CRD，创建和删除部署和服务。 |用于部署和服务的 RBAC |UI 中创建的每个 LangGraph 部署都会在 `langsmith` 命名空间中生成一个 Kubernetes 部署，其中 pod 作为 `langsmith-ksa` ServiceAccount 运行。该 ServiceAccount 必须携带 `iam.gke.io/gcp-service-account` 注释，`deploy.sh` 幂等地应用。
...
* 无论级别如何，GCS 存储桶始终是必需的。

### 秘密管理器（可选模块）

* `enable_secret_manager_module = true` 时存储单个 JSON 秘密包（生成的 LangSmith 秘密密钥、Postgres 密码、Redis 密码）。
...
## 工作负载身份GKE Pod 通过 Workload Identity 访问 GCS。 Kubernetes ServiceAccount 通过 IAM 绑定与 GCP 服务帐户绑定； Pod 接收临时凭证，Secret 或环境变量中没有静态密钥。
...
| `langsmith-ksa`（操作员吊舱）|相同| GCS `storage.objectAdmin` |GSA 由`iam` 模块定义并输出为`workload_identity_annotation`。 `init-values.sh` 自动将这些注释写入`values-overrides.yaml`。
...
`infra` 图层不安装 LangSmith 图。应用程序阶段以两种方式之一安装它，两者都使用 `helm/values/` 下的相同分层值文件：

* 部署脚本：`make init-values && make deploy`运行`helm upgrade --install`。
