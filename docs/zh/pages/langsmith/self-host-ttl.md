...
您可以通过 helm 或环境变量设置来配置保留。有一些可配置的选项：- *已启用：* 是否启用或禁用数据保留。如果启用，您可以通过 UI 将默认组织和项目 TTL 层应用于跟踪（有关详细信息，请参阅[data retention guide](/langsmith/usage-and-billing#data-retention)）。
...
该作业逐表进行，扫描部分并从包含最少数量的过期行的部分中删除数据。该阈值平衡了效率和彻底性：
