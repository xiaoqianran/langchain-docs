...
<Info>
  本指南假设您已经部署了一个自托管的 LangSmith 实例。如果还没有，请参阅[kubernetes deployment guide](/langsmith/kubernetes)。
</Info>

### 配置您想要与 LangSmith 一起使用的应用程序

LangSmith 有一个 API 用于与集线器和 LangSmith 后端进行交互。
...
您还可以直接在 LangSmith SDK 客户端中配置这些变量：
...
设置上述内容后，您应该能够运行代码并在自托管实例中查看结果。我们建议浏览 [*quickstart guide*](https://docs.smith.langchain.com/#quick-start) 以了解如何使用 LangSmith。
