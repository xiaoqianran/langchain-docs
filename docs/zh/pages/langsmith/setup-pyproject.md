...
* `requirements.txt`：对于依赖管理，请查看[this how-to guide](/langsmith/setup-app-requirements-txt)，了解如何使用`requirements.txt` for LangSmith。
* monorepo：要部署位于 monorepo 内的图，请查看 [this repository](https://github.com/langchain-ai/langgraph-example-monorepo) 了解如何执行此操作的示例。

每个步骤之后，都会提供一个示例文件目录来演示如何组织代码。## 指定依赖关系
...
<Tip>
  默认情况下，LangSmith 遵循 `uv`/`pip` 行为，除非明确允许，否则**不**安装预发布版本。如果想使用预发行版，您有以下选择：

  * 对于 `pyproject.toml`：将 `allow-prereleases = true` 添加到您的 `[tool.uv]` 部分。
