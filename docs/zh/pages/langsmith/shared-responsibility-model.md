...
LangSmith是一款多租户SaaS产品。在共享责任模型下，LangChain 保护平台基础设施和应用程序的安全，并且您可以保护您的使用、数据输入和您构建的代理。

有关自带云 (BYOC) 等效项，请参阅 [BYOC shared responsibility model](/langsmith/byoc-shared-responsibility)。
...
## LangChain 职责（平台）- 我们保持 SOC 2 Type II、GDPR 和 HIPAA 合规性，并接受年度第三方审核和渗透测试。
