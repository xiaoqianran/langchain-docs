...
<Tabs>
  <Tab title="Python">
    `run_ids=` 采用运行 ID 的简单列表。 `runs=` 获取每次运行的完整查找键 — 从已有的运行对象中读取 `run_id`、`session_id` 和 `start_time`。<Tabs sync={false}>
      <Tab title="Before">
        ```python Before
...
## 分享和阅读公开跑步共享跟踪、删除其公共访问权限或读取公开共享跟踪中的运行。 v2 方法使用显式 SmithDB 坐标并返回选择驱动的运行对象。
...
### 示例这些示例在读取点之前查询公共跟踪，因为 `public.runs.retrieve` 需要运行的精确存储的 `start_time`。
...
```
  ...
  <Tab title="Go">
    除了`RunID`之外，`Feedback.New`现在还需要`SessionID`。<Tabs sync={false}>
      <Tab title="Before">
        ```go Before
package main
