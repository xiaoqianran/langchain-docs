...
将 LangSmith SDK 运行查询方法迁移到 SmithDB 支持的等效方法。
...
<Tabs>
  <Tab title="Python">
    ...
    | `reference_example_id` | `reference_examples` |现在获取 UUID 列表 || `query` | *（已删除）* |没有同等的 |
    | `filter` | `filter` |语法不变 |
    | `trace_filter` | `trace_filter` |不变 |
    | `tree_filter` | `tree_filter` |不变 |
    | `is_root` | `is_root` |不变 |
    | `parent_run_id` | *（已删除）* |没有同等的 |
    | `start_time` | `min_start_time` |更名；默认为 1 天前 - 请参阅上面的警告 || `error` | `has_error` |更名|
    | `run_ids` | `ids` |更名|
  ...
  <Tab title="Go">
    ...
    | *（不可用）* | `PageSize` |每个请求结果计数（默认 100，最大 1000）|| *（不可用）* | `ReferenceDatasetID` | `ProjectIDs` 的替代方案 |
  </Tab>
...
<Tabs>
  ...
  <Tab title="cURL">
    ...
    | `prompt_cost_details` | `prompt_cost_details.raw` | Field 现在包裹了对象；阅读 `.raw` 以获得相同的 `{category: cost}` 映射，现在带有数字值（是字符串）|
...
<Tabs>
  ...
  <Tab title="cURL">
    `start_time` 已重命名为 `min_start_time`，并且 `run_type` 值现在为大写 (`"llm"` → `"LLM"`)。<Tabs>
      <Tab title="Before">
        ```bash theme={"theme":{"light":"catppuccin-latte","dark":"catppuccin-mocha"}}
        PROJECT_ID=$(curl -s "https://api.smith.langchain.com/api/v1/sessions?name=default&limit=1" \
          -H "x-api-key: $LANGSMITH_API_KEY" | jq -r '.[0].id')
