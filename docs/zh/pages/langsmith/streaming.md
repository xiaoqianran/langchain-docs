...
<Note>
LangGraph SDK 和代理服务器是[LangSmith](/langsmith/observability) 的一部分。
</Note>
...
</Tabs>如果您不需要保留运行的输出，则可以在流式传输时传递 `None` 而不是 `thread_id`。
</Note>
...
线程流支持三种流模式来控制返回哪些事件。通过 `stream_mode` 参数传递一种或多种模式。|模式|描述 |
|------|-------------|
...
线程流通过 `Last-Event-ID` 标头支持可恢复性。如果连接断开，请传递您收到的最后一个事件的 ID 以恢复，而不会丢失事件。通过`"-"`从头开始重播。
