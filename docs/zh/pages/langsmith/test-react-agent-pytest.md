...
<!-- langchain-docs: Test a ReAct agent with Pytest/Vitest and LangSmith | https://docs.langchain.com/langsmith/test-react-agent-pytest -->

# 使用 Pytest/Vitest 和 LangSmith 测试 ReAct 代理

本教程将向您展示如何使用 LangSmith 与流行测试工具（Pytest、Vitest 和 Jest）的集成来评估您的 LLM 申请。我们将创建一个 ReAct 代理来回答有关公开交易股票的问题，并为其编写一个全面的测试套件。

## 设置

本教程使用 [LangGraph](https://langchain-ai.github.io/langgraph/tutorials/introduction/) 用于代理编排，[OpenAI's GPT-4o](https://platform.openai.com/docs/models#gpt-4o)、[Tavily](https://tavily.com/) 用于搜索，[E2B's](https://e2b.dev/) 代码解释器和 [Polygon](https://polygon.io/stocks) 来检索股票数据，但只需稍作修改即可适用于其他框架、模型和工具。 Tavily、E2B 和 Polygon 均可免费注册。
...
为了定义我们的 React 代理，我们将使用 LangGraph/LangGraph.js 进行编排，使用 LangChain 进行 LLM 和工具。
...
现在我们已经定义了代理，让我们编写一些测试来确保基本功能。在本教程中，我们将测试代理的工具调用能力是否有效，代理是否知道忽略不相关的问题，以及是否能够回答涉及使用所有工具的复杂问题。
...
请记住还将 Vitest 和 Jest 的配置文件添加到您的项目中。
