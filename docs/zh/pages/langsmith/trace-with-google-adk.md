...
本指南向您展示如何在 LangSmith 中追踪 [Google Agent Development Kit (ADK)](https://github.com/google/adk-python) 特工。您将为 ADK 应用程序配置自动跟踪，以捕获代理调用、工具调用和 LLM 交互。
...
该函数接受以下可选参数：- `project_name`：要发送跟踪的LangSmith项目。默认为 `LANGSMITH_PROJECT` 环境变量。
...
该集成自动跟踪多代理工作流程，包括顺序和并行代理组合：{/* 由 ls-integration-examples 生成的代码。请勿编辑。 */}
{/* 来源：https://github.com/langchain-ai/ls-integration-examples/tree/main/integrations/google-adk/ */}
```python
import asyncio
