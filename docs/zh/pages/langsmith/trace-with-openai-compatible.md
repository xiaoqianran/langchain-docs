...
许多 LLM 提供商接受与 OpenAI API 相同格式的请求。要跟踪从这些提供程序到 LangSmith 的调用，请构造一个指向提供程序基本 URL 的 OpenAI 客户端，然后用 [⟦T7⟧](https://reference.langchain.com/python/langsmith/wrappers/_openai/wrap_openai) / [⟦T8⟧](https://reference.langchain.com/javascript/modules/langsmith.html) 包装它。
...
<Note>直接追踪OpenAI，请参考[Trace OpenAI applications](/langsmith/trace-openai)。</Note>
...
## 相关指南一些提供商有专门的设置指南，使用 `@traceable` 或本机回调。这些方法在功能级别进行跟踪，而不是直接包装客户端，或者与提供商自己的 SDK 和路由层集成。

- [DeepSeek](/langsmith/trace-deepseek)：OpenAI兼容API；指南使用 `@traceable` 和自定义提供者元数据
- [LiteLLM](/langsmith/trace-litellm)：公开OpenAI兼容端点的代理；指南涵盖 `@traceable` 和 LiteLLM 的内置 LangSmith 回调
