...
- `opencode.assistant.turn` 孩子们跑来寻求助手和模型反应。
- 嵌套工具运行以进行工具调用，包括输入、输出、错误、计时和附件（如果可用）。
- 嵌套在父工具调用下的子代理会话。
...
打开配置的 LangSmith 项目并查找名为 `opencode.session` 的根运行。每个跟踪都包含用户轮流作为根输入和助理响应、工具调用以及子代理跟踪作为子运行。该插件将 OpenCode 会话 ID 存储为 `thread_id` 元数据，因此您可以在 LangSmith 中过滤或分组相关的 OpenCode 轮次。
