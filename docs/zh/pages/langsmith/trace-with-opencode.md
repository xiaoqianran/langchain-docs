...
在 LangSmith 中捕获 OpenCode 会话、助理轮流、工具调用和子代理活动。
...
像往常一样运行 OpenCode。该插件将完成的用户轮流发送到配置的 LangSmith 项目。
...
| `LANGSMITH_OPENCODE_ENDPOINT` |没有 | LangSmith SDK默认| LangSmith API URL。回落到`LANGSMITH_ENDPOINT`。                                                        |
| `LANGSMITH_OPENCODE_PROJECT` |没有 | `opencode` |朗史密斯项目名称。回落到`LANGSMITH_PROJECT`。                                                    |
| `LANGSMITH_OPENCODE_METADATA` |没有 | - | JSON 对象合并到根跟踪元数据中。                                                                  || `LANGSMITH_OPENCODE_RUNS_ENDPOINTS` |没有 | - |副本目标的 JSON 数组。                                                                           |
...
| `project` |没有 | `opencode` |朗史密斯项目名称。                                                          |
| `metadata` |没有 | - |对象合并到根跟踪元数据中。                                          || `replicas` |没有 | - |将跟踪复制到的其他 LangSmith 目标。                        |
...
副本对象支持 Snake\_case 和 LangSmith SDK 风格的驼峰命名法字段名称。建议在配置文件中使用snake\_case。

|领域 |描述 |
| ---------------------------------- | --------------------------------------------------------------------------- |
| `api_url` / `apiUrl` |副本目标的 LangSmith API URL。                              |
| `api_key` / `apiKey` |目标工作区的 API 密钥。                                      |
| `project` / `projectName` |目标工作区中的项目名称。                                  |
| `updates` |用于覆盖复制运行的可选运行字段，例如额外的元数据。 |

## 追踪什么该插件监听 OpenCode 聊天和事件挂钩，聚合每个完成的用户回合，并将其作为运行树提交给 LangSmith。

* `opencode.session` root 运行以完成用户轮次。
* `opencode.assistant.turn` 孩子跑来寻求助手和模型的反应。
* 嵌套工具运行用于工具调用，包括输入、输出、错误、计时和附件（如果可用）。
* 子代理会话嵌套在父工具调用下。
* 模型名称、提供者、调用参数、令牌使用以及线程或会话 ID 元数据。
* 用户消息、助手消息、推理块、文件部分以及与助手轮流相关的系统提示。

跟踪完成基于 OpenCode `step-finish` 事件。当 OpenCode 服务器关闭时，该插件还会刷新待处理的跟踪批次。

## 在 LangSmith 中查看痕迹

打开配置好的 LangSmith 项目并查找名为 `opencode.session` 的根运行。每个跟踪包含用户轮流作为根输入和助理响应、工具调用以及子代理跟踪作为子运行。该插件将 OpenCode 会话 ID 存储为 `thread_id` 元数据，因此您可以在 LangSmith 中过滤或分组相关的 OpenCode 轮次。

## 故障排除

如果 LangSmith 中没有出现痕迹：* 使用配置中的`TRACE_TO_LANGSMITH=true`或`"enabled": true`确认跟踪已启用。
* 确认 LangSmith API 密钥已在 OpenCode 使用的同一 shell、项目配置或全局配置中设置。
* 确认插件包已安装在 OpenCode 可以解析的位置。
* 检查所选的 LangSmith 项目。如果未配置项目，跟踪将转到`opencode`。
* 更改`opencode.json`、`langsmith.json`或环境变量后重新启动OpenCode。
