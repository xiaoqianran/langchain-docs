...
跟踪 Pi 编码代理对 LangSmith 的调用，以观察轮次、调试工具调用、跟踪令牌使用情况并检查各个 LLM 调用。

本指南向您展示如何使用 `@langchain/langsmith-pi-extension` 扩展跟踪到 LangSmith 的 [Pi coding agent](https://pi.dev) 会话。

配置完成后，每个 Pi 会话都会向 LangSmith 发送跟踪信息。每个跟踪都包括用户消息、助理响应、工具调用和单独的 LLM 调用，这使您可以全面观察 Pi 编码代理的运行情况。
...
＃＃ 配置配置可以来自环境变量或 JSON 配置文件。值按以下顺序合并，后面的源优先：

1. 默认值
2. `~/.pi/langsmith.json`（全局配置）
3. `<current-working-directory>/.pi/langsmith.json`（项目配置）
4.环境变量
...
| `LANGSMITH_PI_ENDPOINT` |用于自托管或自定义部署的 LangSmith API URL。回落到`LANGSMITH_ENDPOINT`。                        |
| `LANGSMITH_PI_PROJECT` |朗史密斯项目名称。回落到`LANGSMITH_PROJECT`。默认为`pi-coding-agent`。                           |
| `LANGSMITH_PI_METADATA` |添加到根运行元数据的 JSON 对象。回落到`LANGSMITH_METADATA`。                                     || `LANGSMITH_PI_RUNS_ENDPOINTS` |副本运行目标的 JSON 数组。回落到`LANGSMITH_RUNS_ENDPOINTS`。                                   |
...
| `project` |没有 | `pi-coding-agent` |朗史密斯项目名称。                                                          |
| `metadata` |没有 | — |对象合并到根跟踪元数据中。                                          || `replicas` |没有 | — |要将跟踪复制到的其他 LangSmith 目标的数组。               |

## 副本

使用 `replicas` 同时将跟踪发送到多个 LangSmith 目的地。这对于将跟踪转发到个人工作区和共享团队项目，或者转发到 [cloud](/langsmith/cloud) 旁边的 [self-hosted LangSmith](/langsmith/self-hosted) 实例非常有用。
