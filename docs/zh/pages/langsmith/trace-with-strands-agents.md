...
使用 LangSmith 跟踪 Strands Agents 应用程序。

[Strands Agents](https://strandsagents.com/)是一个用于构建模型驱动代理的SDK。 LangSmith 提供了 Strands Agents 集成，以与 LangSmith 兼容的格式导出 Strands OpenTelemetry 跨度，包括代理运行、模型调用、工具调用、提示、完成和令牌使用。

## 安装

安装具有 Strands Agent 支持的 LangSmith：
...
### 3. 创建并运行您的代理配置完成后，Strands Agent 跟踪会自动导出到 LangSmith：
