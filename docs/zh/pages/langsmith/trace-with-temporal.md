...
了解如何使用 OpenTelemetry 跟踪 LangSmith 中的时间工作流程和活动。

[Temporal](https://temporal.io/)是一个持久的执行平台，使开发人员能够构建弹性分布式应用程序。本指南向您展示如何使用 OpenTelemetry 跟踪 LangSmith 中的时间工作流程和活动。
...
<Tabs>
  <Tab title="Go" icon="brand-golang">
    Go 使用 `langsmith-go` SDK 和 Temporal 的 OpenTelemetry 拦截器来自动跟踪工作流程和活动。
    ...
    <Steps>
      ...
      <Step title="Define workflow and activity">
        定义执行活动的工作流。该活动演示了如何为 LangSmith 可见性添加自定义跨度属性：
  ...
  <Tab title="Python" icon="brand-python">
    ...
    <Steps>
      ...
      <Step title="Initialize tracer">
        创建一个 OpenTelemetry `TracerProvider`，并将 OTLP 导出器配置为向 LangSmith 发送跟踪：
        ...
        def init_tracer_provider() -> TracerProvider:
            ...
            return provider
        ```
      </Step><Step title="Define workflow and activity">
        定义工作流类和活动函数。该活动演示了如何为 LangSmith 可见性添加自定义跨度属性：
      ...
      <Step title="Execute workflow">
        在单独的脚本中，使用跟踪拦截器连接到 Temporal 并执行工作流程：
  ...
  <Tab title="TypeScript / JavaScript" icon="brand-javascript">
    ...
    <Steps>
      ...
      <Step title="Initialize tracer">
        创建一个 `NodeTracerProvider`，并将 OTLP 导出器配置为向 LangSmith 发送跟踪：
...
### 痕迹未出现1. **验证API密钥**：确保`LANGSMITH_API_KEY`设置正确
...
* **Python/TypeScript**：确保 OpenTelemetry SDK 使用跟踪传播器正确初始化

### 工作线程关闭挂起

如果跟踪未刷新，请确保您以适当的超时调用 shutdown 方法：
