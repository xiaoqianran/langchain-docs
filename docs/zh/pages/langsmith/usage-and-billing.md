...
了解 LangSmith 跟踪数据保留层、定价、速率限制和使用限制。
...
* **成本**：LangSmith 对数据保留率较低的跟踪收费较低。如需了解更多信息，请了解如何[enforce spend limits](/langsmith/billing#enforce-spend-limits)。
...
2. 从理念上讲，我们希望对可能无法进行有意义交互的痕迹向客户收取较低的费用。我们认为自动升级使我们的定价模型与 LangSmith 带来的价值保持一致，只有具有有意义交互的跟踪才会以更高的费率收费。
...
* **数据集**：数据集具有无限的数据保留期。换句话说，如果将跟踪的输入和输出添加到数据集，它们将永远不会被删除。我们建议，如果您使用 LangSmith 进行数据收集，请利用数据集功能。
...
* LangSmith 迹线（基本电荷）
* LangSmith Traces（扩展数据保留升级）。第一个指标包括所有跟踪，无论层如何。第二个指标仅计算扩展保留跟踪的数量。
...
## 速率限制LangSmith 有速率限制，旨在确保所有用户的服务稳定性。

为了确保访问和稳定性，LangSmith 将在以下情况下响应 HTTP 状态代码 429，指示已超出速率或使用限制：
...
此 429 由我们的应用程序负载均衡器引发，是一种为所有 LangSmith 用户提供的独立于计划层的机制，以确保所有用户的服务连续性。|方法|端点 |限制|窗口|
| ----------------- | ------------- | -----| -------- |
...
LangSmith 允许您设置两个不同的每月限制，反映了上述数据保留指南中讨论的计费指标：
...
延长数据保留跟踪限制有副作用。如果已达到限制，LangSmith 会阻止会创建另一个扩展保留跟踪的操作。例如，您不能再：

1. 运行延长跟踪保留时间的自动化规则
2. 运行延长跟踪保留时间的评估器

不更改跟踪保留的操作不计入延长保留跟踪限制。当达到限制时，您仍然可以在 LangSmith UI 中提交反馈，将运行或线程添加到注释队列，并在禁用保留扩展的情况下运行自动化规则或评估器。
