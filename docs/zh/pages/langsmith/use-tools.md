...
1. [**Built-in tools**](#built-in-tools)：模型提供商提供的预配置工具（如OpenAI和Anthropic），可供使用。当您需要 Web 搜索或代码解释等常用功能时，请使用内置工具。
...
LangSmith 自动将您创建的工具保存到工作区范围的 [tool registry](#manage-tools-with-the-registry)，这使得它们可以在所有提示和会话中重复使用。
