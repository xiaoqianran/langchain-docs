...
要使用 Webhook，请在 API 请求中指定 `webhook` 参数。运行完成后，LangSmith 会向指定的 Webhook URL 发送 `POST` 请求。

例如，如果您的服务器在 `https://my-server.app/my-webhook-endpoint` 侦听 Webhook 事件，请在您的请求中包含以下内容：
...
要包含机密或特定于环境的值而不将其签入配置文件，请使用 `${{ env.VAR }}` 模板语法：
