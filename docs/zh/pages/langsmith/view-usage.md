...
LangSmith 提供了有关 [organization's](/langsmith/administration-overview) 使用情况的多种视图，具体取决于您的 [plan](/langsmith/pricing-plans) 和 [hosting type](/langsmith/platform-setup)。本页介绍了哪些数据可用、每个指标的含义以及适用于 [Self-hosted](/langsmith/self-hosted) 的限制。

## 使用视图

|查看 |在哪里可以找到它 |谁能看到|计划可用性 |
|------|-----------------|----------------|--------------------|
| [Usage graph](#usage-graph) |**企业**：设置 > 使用情况 > 使用情况图表<br></br>**自助服务**：设置 > 计费 > 使用情况图表 |所有组织成员 |所有计划|
| [Granular usage](#granular-usage) | **企业**：设置 > 使用情况 > 细化使用情况<br></br>**自助服务**：设置 > 计费 > 细化使用情况 |所有组织成员 |所有计划|
| [Contract usage](#contract-usage) | **企业**：设置 > 使用情况 > 合同使用情况<br></br>**自助服务**：设置 > 计费 > 合同使用情况 |仅限组织管理员 (`organization:manage`) |仅限企业|
...
粒度使用按**插入时间**（当LangSmith接收并存储跟踪时）记录跟踪，而不是按其在应用程序中运行的时间记录。实际上，这种差异通常可以忽略不计，但以显着延迟发送的跟踪（例如，缓冲的 SDK 上传）可能会出现在比预期更晚的时间段中。
...
|发票和付款管理 |不可用（计费在LangSmith外部处理）|### 自托管的粒度使用

[Self-hosted](/langsmith/self-hosted) 上提供粒度使用，但需要明确选择加入：

- 在 **LangSmith 0.13.12 及更高版本**，默认启用精细使用情况收集。
