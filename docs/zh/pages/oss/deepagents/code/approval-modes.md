...
选择 Deep Agents Code 如何使用手动、自动和 YOLO 模式审查门控工具调用

默认情况下，Deep Agents Code 在运行可能产生后果的操作之前会请求您的批准。这些称为**门控操作**，包括以下内容：
...
## 启用自动Auto 是一个实验性测试版。要使用它，请设置选择加入标志，然后为您的会话选择“自动”。
...
* 手动审批菜单可以为当前线程启用自动。阈值回退可以永久切换到手动或在启用自动的情况下执行一次性审核。
...
* 远程 `--sandbox` 处于活动状态（自动仅适用于未沙盒的本地会话）。
