...
当您使用代理时，它会使用内存优先协议自动将信息存储在 `~/.deepagents/<agent_name>/memories/` 中作为 markdown 文件：

1. **研究**：在开始任务之前在内存中搜索相关上下文
2. **响应**：执行过程中不确定时检查内存
3. **学习**：自动保存新信息以供将来使用
...
- **项目**：任何 git 项目根目录中的 `.deepagents/AGENTS.md` — 当从该项目内运行 Deep Agents 代码时加载。
...
在交互式会话中，直接使用 `/skill:<name>` 斜线命令运行技能：
