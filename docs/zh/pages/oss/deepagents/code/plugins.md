...
使用与独立Deep Agents代码技能相同的技能格式。安装的插件名称成为技能命名空间。有关更多信息，请参阅[Memory and skills](/oss/deepagents/code/memory-and-skills#skills)。
...
该插件必须声明一个非空的`version`。每个条目必须以 `./` 开头，并保留在插件根目录中。更改已安装的插件扩展后，重新启动 dcode。设置功能、支持的注册、信任模型和安全行为请参见[Python extensions](/oss/deepagents/code/extensions)。
