...
<!-- langchain-docs: Comparison with Claude Agent SDK | https://docs.langchain.com/oss/javascript/deepagents/comparison -->

# 与Claude Agent SDK的比较

将 LangChain Deep Agents 与 Claude Agent SDK 进行比较，选择适合您的用例的工具。

本页介绍了 [LangChain Deep Agents](/oss/javascript/deepagents/overview) 与 [Claude Agent SDK](https://platform.anthropic.com/docs/en/agent-sdk/overview) 的比较。两者都是用于构建自定义代理的工具，但它们在执行环境、部署和供应商耦合方面做出了不同的权衡。
...
|                               | **深层特工** | **克劳德代理SDK** || -------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **代理运行的位置** |在沙箱内部或沙箱外部远程执行命令 |沙盒内部 || **执行后端** |可插拔：[local, virtual filesystem, remote sandbox, or custom](/oss/javascript/deepagents/backends) |它运行的沙箱的本地文件系统 |
| **模型提供商** |任何（Anthropic、OpenAI、Google、100 多个其他）|克劳德（人类、基岩、顶点、天蓝色）|| **按提供商/模型调整** | [Harness profiles](/oss/javascript/deepagents/profiles)（测试版）：系统提示、工具、中间件和子代理调整的声明性捆绑包，按提供商或特定模型注册 |在每个模型调用站点的代码中进行配置 |
| **部署** | LangSmith 中的 [Managed Deep Agents](/langsmith/javascript/managed-deep-agents-overview)，或通过 [⟦T0⟧](/langsmith/cli#build) 自托管 [standalone image](/langsmith/deploy-standalone-server) | [Self-host](https://code.claude.com/docs/en/agent-sdk/hosting)。您构建服务器、身份验证和流层。 [Claude managed agents](https://platform.claude.com/docs/en/managed-agents/overview) 是一个单独的产品 |
| **多租户** | [Built-in](/oss/javascript/deepagents/going-to-production#multi-tenancy)：作用域线程、每用户沙箱、RBAC |自己构建 || **许可证** |麻省理工学院 |麻省理工学院（克劳德代码本身是专有的）|
...
Deep Agents 支持两者，并允许您选择 [backend](/oss/javascript/deepagents/backends#quickstart) 将它们连接在一起。实际上，这意味着您可以：* 在沙箱内运行代理（与 Claude Agent SDK 模型相同）。
...
Deep Agents 直接处理这个问题：在工具中配置一个沙箱[per user or per assistant](/oss/javascript/deepagents/going-to-production#lifecycle)，其中包含范围线程、运行历史记录和[RBAC](/oss/javascript/deepagents/going-to-production#team-access-control-rbac)。如果您使用 [LangSmith Sandbox](/langsmith/sandbox-auth-proxy)，您还可以获得一个开箱即用的身份验证代理，以便最终用户可以从沙箱调用第三方 API，而无需为每个用户配置凭据。
...
* **托管：** 在 LangSmith 中使用 [Managed Deep Agents](/langsmith/javascript/managed-deep-agents-overview) 创建、运行和操作深度代理。
