...
代理到[sandbox](/oss/javascript/deepagents/sandboxes)，以便它可以读取，
...
`useStream`配置参见[Going to production](/oss/javascript/deepagents/going-to-production)。
...
1. **具有沙箱后端的深度代理：** 代理获取文件系统工具
   （`read_file`、`write_file`、`edit_file`、`execute`）自动从
   沙箱

2. **自定义 API 服务器：** 通过 `langgraph.json` 的 `http.app` 公开的 Hono 应用程序
   字段，提供前端可以调用的文件浏览端点
...
对于 [multi-tenant](/oss/javascript/deepagents/going-to-production#multi-tenancy) 应用程序，
...
使用 [sandbox backend](/oss/javascript/deepagents/sandboxes) 配置深度代理
如[Execution environment](/oss/javascript/deepagents/going-to-production#execution-environment)中所述。
...
在共享模块中定义`getOrCreateSandboxForThread`。双方代理图
工厂和自定义 API 路由导入它：
...
将代理连接为读取的异步 [graph factory](/langsmith/graph-rebuild)
运行配置中的`thread_id`并将解析后的后端传递给
`createDeepAgent`：
...
### 种子项目文件在代理运行之前，使用 `uploadFiles` / 上传启动文件
...
代理可以读写文件，但前端也需要直接访问
浏览沙箱文件系统。添加自定义 [Hono](https://hono.dev) API 服务器
...
确保前端始终访问当前的正确沙箱
对话，使用与 相同的 `getOrCreateSandboxForThread` 功能
代理后台：
...
<Note>
  ...
  `getOrCreateSandboxForThread`功能。这确保他们始终能够解决

  到给定线程的同一沙箱。线程元数据中的沙箱 ID
  是唯一的事实来源——不需要内存缓存。
</Note>

### 配置`langgraph.json`注册代理图和 API 服务器。 `http.app`字段告诉我们
LangGraph 平台可与默认路线一起为您的自定义路线提供服务。
参见 [application structure](/oss/javascript/langgraph/application-structure) 和
...
聊天面板。它使用 [⟦T52⟧](https://reference.langchain.com/javascript/langchain-react/index/useStream) 进行代理对话和自定义 API
文件浏览的端点。

对于生产部署，请将 `apiUrl` 指向您的
[LangSmith Deployment](/langsmith/deployment)，并通过稳定
每次运行`thread_id`。参见
[Frontend](/oss/javascript/deepagents/going-to-production#frontend) 在
[Going to production](/oss/javascript/deepagents/going-to-production) 对于这些设置
对于[invoking the agent](/oss/javascript/deepagents/going-to-production#invoking-the-agent)
...
|角度| [⟦T73⟧](https://github.com/rars/ngx-diff) | `<ngx-unified-diff>` 与 `[before]` 和 `[after]` |
...
* **发射前添加护栏**。配置
  ...
  [error handling](/oss/javascript/deepagents/fault-tolerance#error-handling)，以及
  [data privacy](/oss/javascript/deepagents/going-to-production#data-privacy)中间件
  用于自主编码代理。
