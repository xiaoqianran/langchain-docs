...
| `--update` |更新现有文档。默认为代码模式。在没有 `--print` 的 TTY 中，流代理输出并在成功时自动退出。 `--init` 和 `--update` 不能组合。  || `--mode <personal\|code>` |选择个人大脑或存储库文档。                                                                                                                                               |
...
* `/api-key`：更新当前提供商API密钥（屏蔽提示）
* `/langsmith-key`：更新或清除 LangSmith 跟踪凭证（屏蔽提示）
