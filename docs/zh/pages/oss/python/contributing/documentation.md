...
<!-- langchain-docs: Contributing to documentation | https://docs.langchain.com/oss/python/contributing/documentation -->

# 为文档做出贡献

我们欢迎对 LangChain 文档做出贡献，包括新功能、[integrations](/oss/python/contributing/publish-langchain)以及对现有文档的改进。
...
<Note>
  所有拉取请求必须链接到维护者批准解决方案的问题或讨论。请参阅我们的[pull request requirements](/oss/python/contributing/overview#pull-request-requirements)。
</Note>
...
操作指南是面向任务的说明，为那些知道自己想要完成什么任务的用户提供指导。操作指南的示例位于 [LangChain](/oss/python/langchain/overview) 和 [LangGraph](/oss/python/langgraph/overview) 选项卡上。
...
<AccordionGroup>
  ...
  <Accordion title="When to create new reference documentation">
    * 满足 [hosted-guide eligibility criteria](/oss/python/contributing/publish-langchain#eligibility-for-hosted-guides) 要求的新集成（每月下载量或精选次数超过 50,000 次）
    * 复杂的配置选项需要详细解释
    * API 更改引入了新的参数或行为
    * 社区经常询问有关特定功能的问题
  </Accordion>
...
教程是较长的分步指南，以自身为基础，引导用户通过特定的实践活动来建立理解。教程通常可以在 [Learn](/oss/python/learn) 选项卡上找到。<Note>
...
**从 API 参考存根到 OSS 文档：**已发布的 Python API 参考中的交叉链接和深度锚点是在此存储库之外生成的。如果从 reference.langchain.com 到 docs.langchain.com 的链接错误或过时，请使用 [open an issue](https://github.com/langchain-ai/docs/issues/new?template=04-reference-docs.yml) 以及源 URL 和目标 URL。
