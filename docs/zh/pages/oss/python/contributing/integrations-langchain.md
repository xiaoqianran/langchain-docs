...
<Warning>
  新集成**不被接受为任何 `langchain-ai` 存储库的 PR**。所有新集成都必须作为独立包发布到 PyPI（例如，`langchain-yourprovider`）。您应该向 `langchain-ai` 存储库开放的唯一 PR 是在文档中列出您已发布的包：下载表的 YAML 行，或者如果您满足 [eligibility criteria](/oss/python/contributing/publish-langchain#eligibility-for-hosted-guides)，则为托管指南。
</Warning>
...
* [**Chat Models**](/oss/python/integrations/chat)：最常用的组件类型
* [**Tools/Toolkits**](/oss/python/integrations/tools)：启用代理功能
* [**Retrievers**](/oss/python/integrations/retrievers)：RAG 应用程序的核心
* [**Embedding Models**](/oss/python/integrations/embeddings)：向量运算的基础
* [**Vector Stores**](/oss/python/integrations/vectorstores)：语义搜索必备
* [**Middleware**](/oss/python/integrations/middleware)：用钩子扩展代理行为
* [**Sandboxes**](/oss/python/deepagents/sandboxes)：使用 Deep Agent 安全运行代码

**不是这些❌**：

* **LLM（文本完成模型）**：已弃用，支持 [Chat Models](/oss/python/integrations/chat)
* [**Document Loaders**](/oss/python/integrations/document_loaders)：维护负担高
* [**Key-Value Stores**](/oss/python/integrations/stores)：使用有限
* **文档转换器**：利基用例
* **模型缓存**：基础设施问题
* **图**：复杂的抽象
* **消息历史**：存储抽象
...
<Steps>
  <Step title="Implement your package">
    ...
    如果适用，请为您的集成实现对 LangChain [standard test](/oss/python/contributing/standard-tests-langchain) 套件的支持并成功运行它们。
  </Step>
  ...
  <Step title="List your integration">
    ...
    <Accordion title="How listing works" icon="book">
      ...
      * [Vector stores](https://github.com/langchain-ai/docs/blob/main/src/oss/python/integrations/vectorstores/TEMPLATE.mdx)
      ...
      有关完整步骤、资格详细信息和拒绝标准，请参阅[Publish an integration](/oss/python/contributing/publish-langchain#make-your-integration-discoverable)。
    </Accordion>
  </Step>

  <Step title="Co-marketing" icon="speakerphone">
    （可选）与LangChain团队共同参与[co-marketing](/oss/python/contributing/comarketing)。
  </Step>
</Steps>
