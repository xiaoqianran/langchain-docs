...
<!-- langchain-docs: Build a content builder agent | https://docs.langchain.com/oss/python/deepagents/content-builder -->
...
本指南演示了如何使用 [Deep Agents](/oss/python/deepagents) 从头开始构建内容编写代理。
...
* [Long-term memory](/oss/python/deepagents/memory) 用于 TODO
* [Skills](/oss/python/deepagents/skills) 用于 TODO
* [Subagents](/oss/python/deepagents/subagents) 用于 TODO
* [Filesystem backends](/oss/python/deepagents/backends) 用于文件读写
* 自定义[tools](/oss/python/langchain/tools)用于搜索和图像生成
...
<Steps>
  <Step title="Add AGENTS.md">
    在项目根目录中创建`AGENTS.md`。
    当您稍后创建代理并将此文件指定为 [memory](/oss/python/deepagents/memory) 参数的一部分时，它会将此文件加载到系统提示符中，以便品牌声音和研究期望适用于每次运行。
  ...
  <Step title="Add subagents.yaml">
    ...
    然后添加以下文本，其中描述了带有 Tavilly 支持的 `web_search` 工具的 `researcher` 子代理、Haiku 模型 ID 以及将结果保存到从主代理委派时指定的路径的说明：
    ...
    稍后在创建深度代理时，该文件将作为参数传递。
  </Step><Step title="Add skills">
...
在项目根目录中创建`content_writer.py`。以下部分按顺序属于一个文件。

<Steps>
  <Step title="Add tools">
    研究人员子代理使用 Tavily 搜索。
    博客和社交工作流程使用 Gemini 图像生成。
    稍后创建代理时，`load_subagents`函数读取`subagents.yaml`并将工具名称解析为这些修饰函数。
  ...
  <Step title="Create the agent">
    使用 [create\_deep\_agent](https://reference.langchain.com/python/deepagents/graph/create_deep_agent) 创建深度代理时，请传递内存路径、技能目录、图像工具、来自 YAML 的子代理以及以示例目录为根的 [FilesystemBackend](/oss/python/deepagents/backends)，以便正确解析 `./AGENTS.md` 和 `./skills/` 等路径。
...
* 阅读 [Subagents](/oss/python/deepagents/subagents)、[Skills](/oss/python/deepagents/skills) 和 [Customization](/oss/python/deepagents/customization) 进行更深入的配置
