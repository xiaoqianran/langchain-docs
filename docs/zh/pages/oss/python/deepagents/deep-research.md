...
<!-- langchain-docs: Build a deep research agent | https://docs.langchain.com/oss/python/deepagents/deep-research -->
...
本指南演示了如何使用 [Deep Agents](/oss/python/deepagents) 从头开始构建多步骤网络研究代理。该代理将研究问题分解为重点任务，将其委托给专门的子代理，并将研究结果综合成一份综合报告。
...
* [Subagents](/oss/python/deepagents/subagents) 用于并行、上下文隔离的研究
* 自定义[tools](/oss/python/langchain/tools)用于网络搜索
* 可选择加入的多步骤规划[planning tool](/oss/python/deepagents/overview#task-planning)
...
<Steps>
  ...
  <Step title="Enable task planning">
    [Task planning](/oss/python/deepagents/overview#task-planning) 是选择加入。研究工作流程使用 `write_todos` 将问题分解为重点任务，因此在创建代理时传递 [⟦T21⟧](https://reference.langchain.com/python/langchain/agents/middleware/todo/TodoListMiddleware)。
  ...
  <Step title="Create the agent">
    将模型初始化和代理创建添加到`agent.py`。选择您的提供商。包括 [⟦T23⟧](https://reference.langchain.com/python/langchain/agents/middleware/todo/TodoListMiddleware)，以便规划工具可用：
