...
当您选择加入 [⟦T20⟧](https://reference.langchain.com/python/langchain/agents/middleware/todo/TodoListMiddleware) 时，深度代理可以公开 **`todos` 状态** 通道。该中间件添加了 `write_todos` 工具，并在代理执行其计划时保留任务进度。当代理执行时，它会更新每个
...
<Note>
  任务计划是可选择的。如果没有 [⟦T27⟧](https://reference.langchain.com/python/langchain/agents/middleware/todo/TodoListMiddleware)，则不存在 `stream.values.todos`。参见[Task planning](/oss/python/deepagents/overview#task-planning)。
</Note>
...
在代理上启用[⟦T35⟧](https://reference.langchain.com/python/langchain/agents/middleware/todo/TodoListMiddleware)。
