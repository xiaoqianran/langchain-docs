...
1. 在代理上注册一个工具，立即调用`interrupt()`将执行推迟到前端。
2. 在前端定义中镜像相同的工具名称和参数字段。
3. 在前端使用`.implement(...)`实现匹配工具并通过
   他们到`useStream({ tools: [...] })`。
4. 当代理调用匹配工具时，客户端处理该操作并
   使用工具结果恢复中断的运行。

## 在代理上注册该工具Playground 定义了一小组遵循相同规则的客户端工具
模式：代理公开工具模式，前端处理实际的
执行。

在服务器上定义立即调用`interrupt()`的普通工具，然后
镜像相同的工具名称和参数字段
前端`tools.ts`文件。
...
每个工具都会使用前端可以处理的结构化有效负载进行中断，然后
返回运行恢复时提供的值。镜像相同的工具名称和
客户端上的架构，以便前端可以附加实现。
...
定义一个与代理的状态模式匹配的 TypeScript 接口并将其传递为
`useStream` 的类型参数，用于对状态值进行类型安全访问：
...
这对于更丰富的 UI 模式尤其有效
[Tool calling](/oss/python/langchain/frontend/tool-calling)，其中每个工具结果可以
渲染为专用卡片而不是原始 JSON。
...
* 需要时添加评论。对于敏感的客户端操作，请将此模式配对
  与[Human-in-the-loop](/oss/python/langchain/frontend/human-in-the-loop)。
