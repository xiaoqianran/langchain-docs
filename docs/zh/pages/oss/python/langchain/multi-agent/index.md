...
<Tip>
  对于内置多代理支持，请使用[Deep Agents](/oss/python/deepagents/overview)：基于 LangChain 构建的更高级别的工具，附带[subagents](/oss/python/deepagents/subagents)、[skills](/oss/python/deepagents/skills)、规划、虚拟文件系统和上下文管理。
</Tip>
...
* <Icon icon="git-branch" /> **并行化**：为子任务生成专门的工作人员并同时执行它们以获得更快的结果。当单个代理有太多[tools](/oss/python/langchain/tools)并且对使用哪个代理做出错误的决定时，当任务需要具有广泛上下文的专业知识（长提示和特定于领域的工具）时，或者当您需要强制执行顺序约束以仅在满足某些条件后解锁功能时，多代理模式特别有价值。

<Tip>
  多智能体设计的核心是**[context engineering](/oss/python/langchain/context-engineering)**——决定每个智能体看到什么信息。系统的质量取决于确保每个代理都能访问其任务所需的正确数据。
</Tip>
...
| ------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| [**Subagents**](/oss/python/langchain/multi-agent/subagents) |主代理作为工具来协调子代理。所有路由都经过主代理，主代理决定何时以及如何调用每个子代理。                                                         |
| [**Handoffs**](/oss/python/langchain/multi-agent/handoffs) |行为根据状态动态变化。工具调用更新状态变量，触发路由或配置更改、切换代理或调整当前代理的工具和提示。 || [**Skills**](/oss/python/langchain/multi-agent/skills) |按需加载专业提示和知识。单个代理保持控制，同时根据需要从技能加载上下文。                                                                    |
| [**Router**](/oss/python/langchain/multi-agent/router) |路由步骤对输入进行分类并将其定向到一个或多个专门代理。结果被综合为综合响应。                                                                 |
| [**Custom workflow**](/oss/python/langchain/multi-agent/custom-workflow) |使用 [LangGraph](/oss/python/langgraph/overview) 构建定制执行流，混合确定性逻辑和代理行为。将其他模式作为节点嵌入到您的工作流程中。                    |
...
* **处理的令牌**：所有调用的[context window](/oss/python/langchain/context-engineering)使用总量。更多令牌 = 更高的处理成本和潜在的上下文限制。
...
</Tabs>**关键见解：** 切换、技能和路由​​器对于单个任务（每个任务 3 个调用）最有效。子代理添加一个额外的调用，因为结果通过主代理流回 - 这一开销提供了集中控制。
...
<Tabs>
  ...
  <Tab title="Handoffs">
    **2 次呼叫 → 总共 5 次*** 咖啡代理从第 1 回合开始**仍然处于活动状态**（状态持续）
...
**关键见解：** 有状态模式（交接、技能）可节省 40-50% 的重复请求呼叫。子代理保持每个请求的一致成本——这种无状态设计提供了强大的上下文隔离，但代价是重复模型调用。

### 多域

> **用户：**“比较 Python、JavaScript 和 Rust 的 Web 开发”每个语言代理/技能包含约 2000 个文档标记。所有模式都可以进行并行工具调用。
...
以下是所有三种情况下的模式比较：<div>
  |图案|一击|重复请求 |      多域 |
  | ------------------------------------------------------------------------ | :------: | :------------: | :--------------------: |
  | [**Subagents**](/oss/python/langchain/multi-agent/subagents) |  4 次通话 |  8 次通话 (4+4) |   5 次调用，9K 代币 |
  | [**Handoffs**](/oss/python/langchain/multi-agent/handoffs) |  3 次通话 |  5 次通话 (3+2) | 7+ 次调用，14K+ 代币 |
  | [**Skills**](/oss/python/langchain/multi-agent/skills) |  3 次通话 |  5 次通话 (3+2) |  3 次调用，15K 代币 |
  | [**Router**](/oss/python/langchain/multi-agent/router) |  3 次通话 |  6 次通话 (3+3) |   5 次调用，9K 代币 |
</div>

**选择图案：**<div>
  |优化 | [Subagents](/oss/python/langchain/multi-agent/subagents) | [Handoffs](/oss/python/langchain/multi-agent/handoffs) | [Skills](/oss/python/langchain/multi-agent/skills) | [Router](/oss/python/langchain/multi-agent/router) |
  | -------------------- | :------------------------------------------------------: | :----------------------------------------------------: | :------------------------------------------------: | :------------------------------------------------: |
  |单个请求 |                                                          |                            ✅ |                          ✅ |                          ✅ |
  |重复请求 |                                                          |                            ✅ |                          ✅ |                                                    |
  |并行执行 |                             ✅ |                                                        |                                                    |                          ✅ ||大上下文域 |                             ✅ |                                                        |                                                    |                          ✅ |
