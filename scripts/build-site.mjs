#!/usr/bin/env node
// LangChain docs static site — EN + zh-CN (machine-translated), modal-docs page form
...
const OFFICIAL = "https://docs.langchain.com";
const BRAND_MARK = "LC";
const PREFERRED = ["oss-python", "langsmith"];
const SYNC_NOTE_EN = "synced daily from docs.langchain.com";
const SYNC_NOTE_ZH = "每日与 docs.langchain.com 同步";
...
const rewriteLinks = createLinkRewriter({
  asset,
  hosts: ["https://docs.langchain.com"],
  rootPrefixes: ["/oss/", "/langsmith/", "/api-reference/", "/langgraph/", "/oss"],
});

function loadPages(rootDir, stripPrefix = "langchain-docs") {
...
function main() {
  ...
  const enPages = loadPages(EN_PAGES, "langchain-docs");
  if (!enPages.length) {
    console.error("No EN pages");
    process.exit(1);
  }
  // Merge zh markdown when present; otherwise fall back to EN body under /zh/
  ...
  try {
    ...
    const llmsResult = writeLlmsArtifacts({
      ...
      brand: "LangChain Docs",
      description: "Unofficial mirror of LangChain / LangSmith documentation (EN + zh-CN).",
