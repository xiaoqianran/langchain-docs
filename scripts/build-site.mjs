#!/usr/bin/env node
// LangChain docs static site (EN; official English only) — modal-docs page form
...
const SYNC_NOTE = "synced daily from docs.langchain.com";
...
function loadPages() {
  ...
  for (const abs of files) {
    ...
    md = md.replace(/^<!-- langchain-docs:[\s\S]*?-->\n*/m, "");
...
function main() {
  ...
  const ui = P.enrichUi(
    {
      ...UI,
      homeH1: "Build with LangChain, faster",
      homeLead: "A polished mirror of the official LangChain docs — multi-path navigation, OSS + LangSmith + API reference in one place.",
    },
    "en",
    SYNC_NOTE,
  );
  ...
  try {
    ...
    const llmsResult = writeLlmsArtifacts({
      ...
      description: 'Unofficial mirror of LangChain / LangSmith documentation.',
