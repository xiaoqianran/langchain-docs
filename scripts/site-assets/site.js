/**
 * langchain Docs chrome — paradigm v3 (aligned to modal-docs)
...
(function () {
  ...
  const STORE_KEY = "lc-docs-nav-v3";
  const LANG_KEY = "lc-docs-lang";
  ...
  if (active) {
    ...
    if (!anyRestored) {
      const preferred =
        nav?.querySelector('.track[data-track="oss-python"]') ||
        nav?.querySelector('.track[data-track="langsmith"]') ||
